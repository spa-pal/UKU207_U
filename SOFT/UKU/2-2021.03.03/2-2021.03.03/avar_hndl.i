...
unsigned rki_avar1_stat;	 	
unsigned rki_avar1_ind_stat; 	
...
extern signed short net_U,net_Ustore, net_Umax, net_Ustore_max; 
...
if(in==1)
...
else if(in==0 || in==4)	
	{
		...
		if(in==0) net_av&=~1;
		if(in==4) net_av&=~2;	 
		if(net_av==0) if(AV_OFF_AVT) avar_ind_stat = ( (avar_ind_stat & ~((0xffffffff>>(32-1))<<0)) | (0 << 0) );
...
avar_unet_hndl_lbl1:
	lc640_adr=1024+(event_ptr_find*32);
     ...
     if( !( (data[0]=='P')&&(data[1]==0)&&( (in==0 && data[2]=='A' )||(in==4 && data[2]=='B') ) ) )	
     	{        
...
ke_date[0]=lc640_read_long(0x10+500+150+8);
lc640_write_long_ptr(lc640_adr+8,(char*)&ke_date[0]);
ke_date[1]=lc640_read_long(0x10+500+150+12);	
...
void speedz_mem_hndl(unsigned short in)	 
{
char data[4];
unsigned int event_ptr=0,lc640_adr,event_ptr_find=0,event_cnt;
...
void uz_mem_hndl(unsigned short in)	 
{
char data[4];
unsigned int event_ptr=0,lc640_adr,event_ptr_find=0,event_cnt;
...
void fz_mem_hndl(unsigned short in)	 
{
char data[4];
unsigned int event_ptr=0,lc640_adr,event_ptr_find=0,event_cnt;
