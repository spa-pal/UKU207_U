...
unsigned char asymmetry;
unsigned short v_plus, v_minus, u_asymmetry, Ubus;	
unsigned int sk1_24;
unsigned short Iddt_porog_pred, Iddt_porog_error, Iddt[24] ;
...
unsigned char ddt_error; 
unsigned short status_izm_r;	
unsigned int sk_alarm ; 
...
unsigned char sk1_24_table[24], sk_alarm_table[24], ddt_error_table[8]; 
...
unsigned char count_reg_enmv, count_bit_enmv, enmv_puts_en, delay_enmv_puts;  


unsigned char enmv_modbus_adress[8], cnt_enmv_modbus_adress;


unsigned char LVBD_status, lvbd_num_alarm_status;
unsigned char no_lvbd; 
unsigned short lvbd_Uload, lvbd_Uakb;
unsigned char  ver_soft_lvbd;
unsigned short LVBD_porog_U1, LVBD_porog_U2, LVBD_Uload_rele_en, LVBD_Uakb_rele_en;
unsigned short LVBD_speed_rs485;
unsigned char LVBD_adress_rs485, LVBD_mode_rele_enable, LVBD_num_meas;
unsigned char count_mess_lvbd, command_lvbd;
unsigned short data_lvbd; 
...
signed short UVENTOFF;			
signed short VZ_KIND;			
								
signed short SNTP_ENABLE;
signed short SNTP_GMT;

signed short NUMBAT_FSO;
#line 268 "main.c"
...
signed short BAT_C_POINT_1_6;  	
signed short BAT_C_POINT_1_2;  	
signed short BAT_C_POINT_1;		
signed short BAT_C_POINT_3;		
signed short BAT_C_POINT_5;		
signed short BAT_C_POINT_10;	
signed short BAT_C_POINT_20;	
signed short BAT_U_END_1_6;  	
signed short BAT_U_END_1_2;  	
signed short BAT_U_END_1;  		
signed short BAT_U_END_3;  		
signed short BAT_U_END_5;  		
signed short BAT_U_END_10;  	
signed short BAT_U_END_20;  	
...
signed short net_U,net_Ustore,net_Ua,net_Ub,net_Uc, net_Umax, net_Ustore_max; 
char bFF,bFF_;
...
{
if (((SysTick_Type *) ((0xE000E000UL) + 0x0010UL))->CTRL & 0x10000) 
     {
     timer_tick ();
     tick = 1;
     }
...
lc640_write_int(0x10+350+26,1);
lc640_write_int(0x10+350+28,7);

lc640_write_int(0x10+500+200,1);
lc640_write_int(0x10+500+200+2,0);
...
if((cnt_net_drv>=0)&&(cnt_net_drv<=NUMIST)) 
	...
	if(!bCAN_OFF)
		if(avar_bps_reset_cnt || bps[cnt_net_drv]._apv_reset_av_timer)  mcp2515_transmit(cnt_net_drv,cnt_net_drv,0x16,0x63,0,0,0,0);
		else mcp2515_transmit(cnt_net_drv,cnt_net_drv,0xED,bps[cnt_net_drv]._flags_tu,*((char*)(&bps[cnt_net_drv]._vol_u)),*((char*)((&bps[cnt_net_drv]._vol_u))+1),*((char*)(&bps[cnt_net_drv]._vol_i)),*((char*)((&bps[cnt_net_drv]._vol_i))+1));
...
#line 1516 "main.c"
    ...
    if(ibat_metr_cnt<60)ibat_metr_cnt+=3;
	}

else if(cnt_net_drv==-1)
	{
    if(!bCAN_OFF)mcp2515_transmit(0x69,0x69,bdr_transmit_stat,bdr_transmit_stat,*((char*)((&UMAX))+1),*((char*)(&DU)),*((char*)((&DU))+1),0);
    plazma_pavlik++;
	}



else if( num_net_in && cnt_net_drv==-3) 
	{
     if(!bCAN_OFF)
		{
...
else if( num_rki && cnt_net_drv==-2) 
	{
     if(!bCAN_OFF)
		...
		if(no_rki<15) ++no_rki;
			...
			u_asymmetry=0;  	
			Ubus=0;				
			for(v=0;v<24;v++){	sk_alarm_table[v]=0; sk1_24_table[v]=0;	} 
			asymmetry=0;
			Iddt_porog_pred=0;
			Iddt_porog_error=0;
			asymmetry_porog=0;
...
else if( NUMLVBD && cnt_net_drv==-10) 
	{
     if(!bCAN_OFF)
		{
		unsigned char ips_stat=0;
		...
		if((kb_full_ver)||(kb_cnt_2lev)||(kb_cnt_1lev)) ips_stat|=0x01; 
		else ips_stat&=0xFE;
		if(avar_stat&0x01) ips_stat|=0x02; 
		else ips_stat&=0xFD;
		mcp2515_transmit(0xE8,0xE8,count_mess_lvbd,command_lvbd,ips_stat,(char)(data_lvbd>>8),(char)data_lvbd,0);
		command_lvbd=0;
		++count_mess_lvbd;
		if(count_mess_lvbd>3) count_mess_lvbd=0;	
		if(no_lvbd<15) ++no_lvbd;
		else{ 
			 LVBD_adress_rs485=0;
			 LVBD_mode_rele_enable=0;
			 LVBD_num_meas=0;
			 LVBD_speed_rs485=0;
			 LVBD_porog_U1=0;
			 LVBD_porog_U2=0;
			 LVBD_Uload_rele_en=0;
			 LVBD_Uakb_rele_en=0;
			 ver_soft_lvbd=0;
			 lvbd_Uload=0;
			 lvbd_Uakb=0;
...
if(a_ind . i==iMn_3U)
	...
	ptrs[12+NUMIST+NUMBAT+NUMMAKB+(NUMEXT!=0)]=		" Версия ПО          ";	 
	ptrs[13+NUMIST+NUMBAT+NUMMAKB+(NUMEXT!=0)]=		" Тест               ";
	ptrs[14+NUMIST+NUMBAT+NUMMAKB+(NUMEXT!=0)]=		" Специнформация     ";
	...
	ptrs[23+NUMBAT]=  											" БПС N19            ";
    ptrs[24+NUMBAT]=  											" БПС N20            ";
    ptrs[25+NUMBAT]=  											" БПС N21            ";
    ptrs[26+NUMBAT]=  											" БПС N22            ";
    ptrs[27+NUMBAT]= 											" БПС N23            ";
    ptrs[28+NUMBAT]= 											" БПС N24            ";
    ptrs[29+NUMBAT]= 											" БПС N25            ";
    ptrs[30+NUMBAT]= 											" БПС N26            ";
    ptrs[31+NUMBAT]= 											" БПС N27            ";
    ptrs[32+NUMBAT]= 											" БПС N28            ";
    ptrs[33+NUMBAT]= 											" БПС N29            ";
    ptrs[34+NUMBAT]= 											" БПС N30            ";
    ptrs[35+NUMBAT]= 											" БПС N31            ";
    ptrs[36+NUMBAT]= 											" БПС N32            ";
	...
	ptrs[6+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+NUMBAT]= " Модуль LVBD        ";

	ptrs[6+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+NUMBAT+NUMLVBD]= " Ускоренный заряд   ";
	if((sp_ch_stat!=scsOFF)&&(bFL))
	ptrs[6+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+NUMBAT+NUMLVBD]= "                    "; 
 	
	ptrs[7+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+NUMBAT+NUMLVBD]= " Выравнивающий заряд";
    if(((hv_vz_stat!=hvsOFF)||(spc_stat==spcVZ))&&(bFL))	
	ptrs[7+NUMIST+(NUMEXT!=0)+(num_net_in!=0)+(num_rki!=0)+NUMBAT+NUMLVBD]= "                    ";
	if(SMART_SPC)
		{
	ptrs[8+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+NUMBAT+NUMLVBD]= " Уравнительный заряд";
	ptrs[9+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+NUMBAT+NUMLVBD]= "с ручным отключением";
	ptrs[10+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+NUMBAT+NUMLVBD]=" нагрузки   включить";
    if((vz1_stat!=vz1sOFF))
		{
		
		
		if(bFL)	ptrs[10+NUMIST+(NUMEXT!=0)+(num_net_in!=0)+(num_rki!=0)+NUMBAT+NUMLVBD]=" нагрузки  выключить";
		else   	ptrs[10+NUMIST+(NUMEXT!=0)+(num_net_in!=0)+(num_rki!=0)+NUMBAT+NUMLVBD]=" нагрузки           ";
		}
	ptrs[11+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+NUMBAT+NUMLVBD]= 	" Формовочный заряд  ";
	ptrs[12+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+NUMBAT+NUMLVBD]= 	"с ручным отключением";
	ptrs[13+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+NUMBAT+NUMLVBD]= 	" нагрузки   включить";
    if((vz2_stat!=vz2sOFF) )
		{
		
		
		if(bFL)	ptrs[13+NUMIST+(NUMEXT!=0)+(num_net_in!=0)+(num_rki!=0)+1+NUMLVBD]=" нагрузки  выключить";
		else   	ptrs[13+NUMIST+(NUMEXT!=0)+(num_net_in!=0)+(num_rki!=0)+1+NUMLVBD]=" нагрузки           ";
		}
		}
	ptrs[8+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+((SMART_SPC)*6)+NUMBAT+NUMLVBD]= 	" Установки          "; 
    ptrs[9+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+((SMART_SPC)*6)+NUMBAT+NUMLVBD]= 	" Журнал событий     "; 
    ptrs[10+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+((SMART_SPC)*6)+NUMBAT+NUMLVBD]= 	" Выход              "; 
    
    
	ptrs[11+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+((SMART_SPC)*6)+NUMBAT+NUMLVBD]=	" Тест               ";
	ptrs[12+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+((SMART_SPC)*6)+NUMBAT+NUMLVBD]=	" Таблица источников ";
	ptrs[13+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+((SMART_SPC)*6)+NUMBAT+NUMLVBD]=	" Версия ПО          ";
	ptrs[14+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+((SMART_SPC)*6)+NUMBAT+NUMLVBD]=	" Специнформация     ";
	ptrs[15+NUMIST+(num_net_in!=0)+(num_rki!=0)+(NUMEXT!=0)+((SMART_SPC)*6)+NUMBAT+NUMLVBD]=	" Контроль емкости   ";
...

 {
...
else if(a_ind . i==iMn_IPS_SGEP_GAZPROM)
	{
	ptrs[0]	=	"                    ";
	if(num_of_wrks_bps)	
		{
	...
	ptrs[4+NUMBAT_FSO]=  						" БПС N1             ";
    ptrs[5+NUMBAT_FSO]=  						" БПС N2             ";
    ptrs[6+NUMBAT_FSO]=  						" БПС N3             ";
    ptrs[7+NUMBAT_FSO]=  						" БПС N4             ";
    ptrs[8+NUMBAT_FSO]= 						" БПС N5             ";
    ptrs[9+NUMBAT_FSO]= 						" БПС N6             ";
    ptrs[10+NUMBAT_FSO]= 						" БПС N7             ";
    ptrs[11+NUMBAT_FSO]= 						" БПС N8             ";
    ptrs[12+NUMBAT_FSO]= 						" БПС N9             ";
    ptrs[13+NUMBAT_FSO]= 						" БПС N10            ";
    ptrs[14+NUMBAT_FSO]= 						" БПС N11            ";
    ptrs[15+NUMBAT_FSO]= 						" БПС N12            ";

    ptrs[4+NUMIST+NUMBAT_FSO]= 				" Сеть               "; 
    ptrs[5+NUMIST+NUMBAT_FSO]= 				" Нагрузка           "; 
    ptrs[6+NUMIST+NUMBAT_FSO]= 				" Автоматы           "; 
	ptrs[7+NUMIST+NUMBAT_FSO]=  			" Внешние датчики    ";
    ptrs[8+NUMIST+NUMBAT_FSO]= 				" Установки          ";
...

 {
	...
	cnt_ind_bat++;
	if(cnt_ind_bat>=(NUMBAT_FSO*40)) cnt_ind_bat=0;
	


 
	 
	if(NUMBAT_FSO==0)sub_bgnd(" Работа без батарей ",'z',-2);
	else
		{
		int2lcd((cnt_ind_bat/40)+1 ,'z',0);
		int2lcd((cnt_ind_bat/40)+1 ,'z',0);
	...
	if(bat[a_ind . s_i1]._av&1 )
		...
		if(UBM_AV)
		ptrs[3]=       " Uбат.с.т.=(^%)   zВ";
		if((bat[a_ind . s_i1]._av&0x02)&&(bFL))ptrs[3]=sm_;
	...
	if(BAT_TYPE==0)	
		...
		else if(lakb[a_ind . s_i1]._s_o_h>=400)ptrs[0]=  "      48V40Ah       ";
		...
		else if(lakb[a_ind . s_i1]._s_o_h>=200)ptrs[0]=  "      48V20Ah       ";
...
else if(a_ind . i==iNet_IPS_SGEP_GAZPROM)
	{
...
#line 6945 "main.c"
...
else if(a_ind . i==iExtern_220_ZVU)
	{
	signed char temp;
	...
	if((av_head[0]=='U')&&(av_head[2]=='R'))
		...
		{
		ptrs[0]=	"       Нагрев       ";
		if(av_head[3]=='3')ptrs[0]=	"   Сильный нагрев   ";  
		ptrs[1]="     батареи N+     ";
		ptrs[2]="  0%(  0^ 0@:0#:0$  ";
		if((av_data_off[0]=='A')&&(av_data_off[1]=='A'))
			{
	...
	||(a_ind . i==iSpc_prl_ke)||(a_ind . i==iAusw_prl)||(a_ind . i==iPrltst)||(a_ind . i==iPrlVZ1)||(a_ind . i==iPrlVZ2)
	||(a_ind . i==iPrl_Def_220_IPS_TERMOKOMPENSAT)||(a_ind . i==iSet_prl_FSO_inf))
	{
...
#line 8454 "main.c"
	...
	ptrs[24]=		" Imin =      {A     ";
	ptrs[25]=		" Uвыр.зар.=   [В    ";
	ptrs[26]=		" Tз.вкл.а.с. !с     ";
	ptrs[27]=		" tи.max=     $°C    ";
	ptrs[28]=		" tи.сигн=    z°C    ";
	ptrs[29]=		" tбат.max=   b°C    ";
	ptrs[30]=		" tбат.сигн=  X°C    ";
	ptrs[31]=		" tвент.вкл.  =  (°C ";
	ptrs[32]=		" tвент.выкл. =  )°C ";
	ptrs[33]=		" Сигнал для вентиля-";
	ptrs[34]=		" тора          >    ";
	ptrs[35]=		" Отключение низко-  ";
	ptrs[36]=		" приоритетной нагр. ";
	ptrs[37]=		" Внешние датчики    ";
	ptrs[38]=		" Ethernet           ";
	ptrs[39]=      	" Серийный N        w";
	ptrs[40]=      	" Тип батареи        ";
	ptrs[41]=      	" Время ротации      ";
	ptrs[42]=      	" источников    lчас.";
	ptrs[43]=      	" Контроль ср.точки  ";
	ptrs[44]=      	" батареи         Q% ";
	ptrs[45]=		" MODBUS ADRESS     d"; 
	ptrs[46]=		" MODBUS BAUDRATE    ";
	ptrs[47]=		"                  f0";	   
   	ptrs[48]=		" Блок допреле       ";

	ptrs[49]=		" Модуль сбора ДС    ";
	ptrs[50]=		" Выход              ";
	ptrs[51]=		" Калибровки         "; 
	ptrs[52]=		"                    ";    
...
#line 9413 "main.c"
	...
	ptrs[12]=		" Umaх.аварбпс=    !В";
    ptrs[13]=		" Umin.аварбпс=    ZВ";
	ptrs[14]=		" Umin.сети=  &В     ";
	ptrs[15]=		" Umax.сети=  FВ     ";
	ptrs[16]=		" Imaх =      ]A     ";
    ptrs[17]=		" Imin =      {A     ";
    ptrs[18]=		" Tз.вкл.а.с. yс     ";
	ptrs[19]=		" tи.maх=     $°C    ";
	ptrs[20]=		" tи.сигн=    Y°C    ";
	ptrs[21]=		" Внешние датчики    ";
    ptrs[22]=      	" Контроль выходного ";
    ptrs[23]=      	" напряжения         ";
	ptrs[24]=      	" Термокомпенс.     q";
	ptrs[25]=      	" Время ротации      ";
	ptrs[26]=      	" источников    lчас.";
	ptrs[27]=      	" Автономная работа  ";
	ptrs[28]=      	" источников         ";
	ptrs[29]=      	" Дополнительное реле";
	ptrs[30]=      	" Блокировка ИПС     ";
	ptrs[31]=      	" Серийный N        w";
	ptrs[32]=		" MODBUS ADRESS     <";
	ptrs[33]=		" MODBUS BAUDRATE    ";
	ptrs[34]=		"                  >0";
	ptrs[35]=		" Ethernet           ";
    ptrs[36]=		" Порог ресурса      ";
    ptrs[37]=		" вентилятора      ^ч";
    ptrs[38]=		" Выравнивание токов ";
	ptrs[39]=		" Стартовый шим    (%";
	ptrs[40]=		" Проверка цепи      ";
	ptrs[41]=		" батареи - )        ";
	ptrs[42]=		" Скорость регулир.  ";
	ptrs[43]=		"    &               ";
	ptrs[44]=		" Блок допреле       ";
   	ptrs[45]=		" РКИ                "; 
	ptrs[46]=		" Сетевые вводы      ";
	
	ptrs[47]=		" LVBD               ";
	ptrs[48]=		" Модуль сбора ДС    ";
	ptrs[49]=		" Спецзаряды с откл.-";
	ptrs[50]=		" нием нагрузки W    ";	 
	ptrs[51]=		" Выход              ";
    ptrs[52]=		" Калибровки         "; 
    ptrs[53]=		"                    ";        
...
else if(a_ind . i==iSet_IPS_SGEP_GAZPROM)
	{
...
#line 10082 "main.c"
	...
	if(a_ind . s_i==2)		ip2lcd(lc640_read_int(4474),lc640_read_int(4476),lc640_read_int(4478),lc640_read_int(4480),'#',(a_ind . s_i1+1));
	else 				ip2lcd(lc640_read_int(4474),lc640_read_int(4476),lc640_read_int(4478),lc640_read_int(4480),'#',0);
	...
	ptrs[7]=" Модуль сбора ДС   ]";	 
	ptrs[8]=" Блоков доп.реле   ^";	  
	ptrs[9]=" Выход              ";
...
#line 10613 "main.c"
...
else if(a_ind . i==iStr_IPS_SGEP_GAZPROM)
	{
	ptrs[0]=" Источников        !";
	ptrs[1]=" Выход              ";
	ptrs[2]="                    ";
	...
	if(a_ind . s_i==21)community2lcd(sss,'>',a_ind . s_i1,1);
	else community2lcd(sss,'>',a_ind . s_i1,0);
...
else if(a_ind . i==iK_IPS_SGEP_GAZPROM)
	{
	char i;
	i=0;
...
else if(a_ind . i==iK_bat_sel_FSO)
	{
	ptrs[0]=						" Батарея N1         ";
    ptrs[1]=						" Батарея N2         ";
    ptrs[0+NUMBAT_FSO]=		" Выход              ";
	ptrs[1+NUMBAT_FSO]=		"                    ";
	ptrs[2+NUMBAT_FSO]=		"                    ";
...
if(a_ind . i==iDeb)
     ...
     if(a_ind . s_i==0)
		...
		int2lcdyx(bps[a_ind . s_i1  ]._vol_u,1,12,0);
		int2lcdyx(bps[a_ind . s_i1+1]._vol_u,2,12,0);
		int2lcdyx(bps[a_ind . s_i1+2]._vol_u,3,12,0);		
...
#line 13150 "main.c"
		...
		long2lcdyx_mmm(lc640_read_int(0x10+350+90),3,13,0);
		...
		int2lcdyx(lc640_read_int(0x10+350+88),0,19,0);
...
if(uku_or_rki==0){	
	...
	ptrs[16+NUMIST]=				"                    ";
	 ...
	 if(NUMBDR)
		{
		ptrs[13+NUMIST]=			" Реле выносного     ";
    	ptrs[14+NUMIST]=			" блока N1          {";
		ptrs[15+NUMIST]=			" Реле выносного     ";
    	ptrs[16+NUMIST]=			" блока N2          }";
		ptrs[17+NUMIST]=			" Реле выносного     ";
    	ptrs[18+NUMIST]=			" блока N3          [";
		ptrs[19+NUMIST]=			" Реле выносного     ";
    	ptrs[20+NUMIST]=			" блока N4          ]";
		}
	ptrs[13+NUMIST+NUMBDR*8]=				" Сброс              ";               
	ptrs[14+NUMIST+NUMBDR*8]=				" Выход              ";
	ptrs[15+NUMIST+NUMBDR*8]=				"                    ";
	ptrs[16+NUMIST+NUMBDR*8]=				"                    ";
	...
	if(tst_state[10]==tst1) sub_bgnd("ВКЛ.",'{',-3);
	if(tst_state[10]==tst2) sub_bgnd("ВЫКЛ.",'{',-4);
	else sub_bgnd("РАБОЧ.",'{',-5);

	if(tst_state[11]==tst1) sub_bgnd("ВКЛ.",'}',-3);
	if(tst_state[11]==tst2) sub_bgnd("ВЫКЛ.",'}',-4);
	else sub_bgnd("РАБОЧ.",'}',-5);
	
	if(tst_state[12]==tst1) sub_bgnd("ВКЛ.",'[',-3);
	if(tst_state[12]==tst2) sub_bgnd("ВЫКЛ.",'[',-4);
	else sub_bgnd("РАБОЧ.",'[',-5);
	
	if(tst_state[13]==tst1) sub_bgnd("ВКЛ.",']',-3);
	if(tst_state[13]==tst2) sub_bgnd("ВЫКЛ.",']',-4);
	else sub_bgnd("РАБОЧ.",']',-5);
	...
	else if((a_ind . s_i==(13+NUMIST))&&(NUMBDR==1))
		{
		if(tst_state[10]==tst1)mess_send(210,117,1,5);
		else if(tst_state[10]==tst2)mess_send(210,117,0,5);
		}
	else if((a_ind . s_i==(15+NUMIST))&&(NUMBDR==1))
		{
		if(tst_state[11]==tst1)mess_send(210,118,1,5);
		else if(tst_state[11]==tst2)mess_send(210,118,0,5);
		}
	else if((a_ind . s_i==(17+NUMIST))&&(NUMBDR==1))
		{
		if(tst_state[12]==tst1)mess_send(210,119,1,5);
		else if(tst_state[12]==tst2)mess_send(210,119,0,5);
		}
	else if((a_ind . s_i==(19+NUMIST))&&(NUMBDR==1))
		{
		if(tst_state[13]==tst1)mess_send(210,120,1,5);
		else if(tst_state[13]==tst2)mess_send(210,120,0,5);
		}
	...
	else if((a_ind . s_i==(8+NUMIST))&&(NUMBDR==1))
		{
		if(tst_state[5]==tst1)mess_send(210,117,1,5);
		else if(tst_state[5]==tst2)mess_send(210,117,0,5);
		}
	else if((a_ind . s_i==(10+NUMIST))&&(NUMBDR==1))
		{
		if(tst_state[6]==tst1)mess_send(210,118,1,5);
		else if(tst_state[6]==tst2)mess_send(210,118,0,5);
		}
	else if((a_ind . s_i==(12+NUMIST))&&(NUMBDR==1))
		{
		if(tst_state[7]==tst1)mess_send(210,119,1,5);
		else if(tst_state[7]==tst2)mess_send(210,119,0,5);
		}
	else if((a_ind . s_i==(14+NUMIST))&&(NUMBDR==1))
		{
		if(tst_state[8]==tst1)mess_send(210,120,1,5);
		else if(tst_state[8]==tst2)mess_send(210,120,0,5);
		}
...
#line 15985 "main.c"
	...
	if(tst_state[5]==tstOFF) mess_send(205,308,a_ind . s_i1,10);
...
#line 16295 "main.c"
	...
	else if(NPN_OUT==npnoBDR) sub_bgnd("Реле БДР",'!',-7); 				 
	else sub_bgnd("Выкл.",'!',-1);
	...
	if(a_ind . s_i1==NUMIST-2)int2lcd(ibat_metr_cnt,'@',0);
	...
	if(no_lvbd==15){
		...
		if(LVBD_status&0x01)ptrs[0]=" Реле  включено     ";
		else				ptrs[0]=" Реле  выключено    ";
		ptrs[1]=					" U нагр/ипс       qВ";
		ptrs[2]=					" U бат            wВ";
		if(LVBD_status&0x0E)ptrs[3]=" Статус:            ";
		else				ptrs[3]=" Статус:     норма  ";
		ptrs[4]=					"                    ";
		ptrs[5]=					"                    ";
		ptrs[6]=					"                    ";	
		ptrs[7]=					"                    ";	 
		lvbd_num_alarm_status=0;
		if(LVBD_status&0x02) {
			lvbd_num_alarm_status=1; 
			ptrs[4]=						"   -авария LVBD     ";
		}
		if(LVBD_status&0x04) {
			++lvbd_num_alarm_status; 
			ptrs[3+lvbd_num_alarm_status]=	"   -низкое Uбат     ";
		}
		if(LVBD_status&0x08) {
			++lvbd_num_alarm_status; 
			ptrs[3+lvbd_num_alarm_status]=	"   -авария реле     ";
		}
		++lvbd_num_alarm_status;
		ptrs[3+lvbd_num_alarm_status]=		" Версия ПО LVBD   s ";
	...
	int2lcd(lvbd_Uload,'q',1);
	int2lcd(lvbd_Uakb,'w',1);
	int2lcd(ver_soft_lvbd,'s',0);

 }

}

else if(a_ind . i==iSetLVBD){	
	ptrs[0]=						" Uипс включ.      #В";
	ptrs[1]=						" Uакб включ.      &В";
    ptrs[2]=						" Порог откл.      $В";
	ptrs[3]=						" Авар. порог      qВ";
	ptrs[4]=						" Задержка       wсек";
	ptrs[5]=						" Режим работы:      ";
	if(LVBD_mode_rele_enable==0){
	ptrs[6]=						" - LVBD             ";}
	else if(LVBD_mode_rele_enable==1){
	ptrs[6]=						" - LVLD             ";}
	else ptrs[6]=					"                    ";
	ptrs[7]=						" Адрес MODBUS     @ ";
	ptrs[8]=						" Скорость        ?0 ";	   	
	...
	bgnd_par(			"   УСТАНОВКИ LVBD   ",
						ptrs[a_ind . i_s],
						ptrs[a_ind . i_s+1],
						ptrs[a_ind . i_s+2]
						);
	pointer_set(1);
	int2lcd(LVBD_Uload_rele_en,'#',1);
	int2lcd(LVBD_Uakb_rele_en,'&',1);
	int2lcd(LVBD_porog_U1,'$',1);
	int2lcd(LVBD_porog_U2,'q',1);	  
	int2lcd(LVBD_num_meas,'w',0);
	int2lcd(LVBD_adress_rs485,'@',0);
	int2lcd(LVBD_speed_rs485,'?',0); 
...
else if(a_ind . i==iK_LVBD){ 
		ptrs[0]=					" U нагр/ипс       qВ";
		ptrs[1]=					" U бат            wВ";
		ptrs[2]=					" Выход              ";
		
	if((a_ind . s_i-a_ind . i_s)>2)a_ind . i_s=a_ind . s_i-2;
	else if(a_ind . s_i<a_ind . i_s)a_ind . i_s=a_ind . s_i;
	bgnd_par(			"   КАЛИБРОВКА LVBD  ",
						ptrs[a_ind . i_s],
						ptrs[a_ind . i_s+1],
						ptrs[a_ind . i_s+2]
						);
...
else if(a_ind . i==iSetENMV){	
	ptrs[0]=						" Адрес ДС №1      ! ";
	if(NUMENMV<2) ptrs[1]=			"                    ";
	else ptrs[1]=					" Адрес ДС №2      @ ";
    if(NUMENMV<3) ptrs[2]=			"                    ";
	else ptrs[2]=					" Адрес ДС №3      # ";
	ptrs[3]=						" Адрес ДС №4      $ ";
	ptrs[4]=						" Адрес ДС №5      % ";
	ptrs[5]=						" Адрес ДС №6      ^ ";
    ptrs[6]=						" Адрес ДС №7      & ";
	...
	bgnd_par(			" УСТАНОВКИ модуля ДС",
						ptrs[a_ind . i_s],
						ptrs[a_ind . i_s+1],
						ptrs[a_ind . i_s+2]
						);
	pointer_set(1);
	int2lcd(enmv_modbus_adress[0],'!',0);  
	int2lcd(enmv_modbus_adress[1],'@',0);
	int2lcd(enmv_modbus_adress[2],'#',0);
	int2lcd(enmv_modbus_adress[3],'$',0);	  
	int2lcd(enmv_modbus_adress[4],'%',0);
	int2lcd(enmv_modbus_adress[5],'^',0);
	int2lcd(enmv_modbus_adress[6],'&',0);
	int2lcd(enmv_modbus_adress[7],'*',0); 
	...
	int2lcd(a_ind . s_i1+1,'!',0); 
 	checkboxing('@',RELE_SET_MASK[a_ind . s_i1]&(1<<0));
	checkboxing('#',RELE_SET_MASK[a_ind . s_i1]&(1<<1));
 	checkboxing('$',RELE_SET_MASK[a_ind . s_i1]&(1<<2));
	checkboxing('%',RELE_SET_MASK[a_ind . s_i1]&(1<<3));
 	checkboxing('^',RELE_SET_MASK[a_ind . s_i1]&(1<<4));
	checkboxing('[',RELE_SET_MASK[a_ind . s_i1]&(1<<5));
	checkboxing('{',RELE_SET_MASK[a_ind . s_i1]&(1<<6));
	checkboxing('}',RELE_SET_MASK[a_ind . s_i1]&(1<<7));
	if(RELE_SET_MASK[a_ind . s_i1]&(1<<15))		sub_bgnd("ВКЛ.",'&',-3);
	else 									sub_bgnd("ВЫКЛ.",'&',-4);
...
else if(a_ind . i==iRele_set_6U)
	{	
	ptrs[0]=	" АБ разряжена      @";
	ptrs[1]=	" Выравнивающий      "; 
	ptrs[2]=	" заряд             #";
    ptrs[3]=	" Общая авария ИБЭП $";		
    ptrs[4]=	" Ток АКБ < -0.5А   %";
	ptrs[5]=	" Отключение НПН    ^";
	ptrs[6]=	" К.Е. АКБ №1       [";
	ptrs[7]=	" К.Е. АКБ №2       {";
	ptrs[8]=	" Активное состояние ";
	ptrs[9]=	"  реле             &";
    ptrs[10]=	sm_exit;
    ptrs[11]=	sm_;
    ptrs[12]=	sm_;     	     	    
	...
	int2lcd(bps_I,'(',1);
	int2lcd_mmm(lc640_read_int(0x10+350+90),')',1);
...
else if(a_ind . i==iSet_bat_point)
	{
	char ptr_ptrs=0;
    ptrs[ptr_ptrs++]=  	    " Количество 2В      ";
	ptrs[ptr_ptrs++]=  	    " элементов      n   ";
	ptrs[ptr_ptrs++]=  	    " C20           &А*ч ";
	ptrs[ptr_ptrs++]=  	    " U20           (В   "; 
	ptrs[ptr_ptrs++]=  	    " C10           ^А*ч ";
	ptrs[ptr_ptrs++]=  	    " U10           FВ   ";    
	ptrs[ptr_ptrs++]=  	    " C5            %А*ч ";
	ptrs[ptr_ptrs++]=  	    " U5            fВ   ";
	ptrs[ptr_ptrs++]=		" C3            $А*ч ";
	ptrs[ptr_ptrs++]=  	    " U3            WВ   ";
	ptrs[ptr_ptrs++]=		" C1            #А*ч ";
	ptrs[ptr_ptrs++]=  	    " U1            wВ   "; 
	ptrs[ptr_ptrs++]=  	    " C1/2          @А*ч ";
	ptrs[ptr_ptrs++]=  	    " U1/2          QВ   ";
	ptrs[ptr_ptrs++]=  	    " C1/6          !А*ч ";
...
if(but==249)
...
else if((but==254)&&(hv_vz_stat==hvsSTEP3))
	{
	hv_vz_stat=hvsWRK;
	...
	if(but==239)
		...
		a_ind . s_i++;
		a_ind . i_s=0;
		gran_ring_char(&a_ind . s_i,0,29);
		}
	else if(but==247)
		{
		a_ind . s_i--;
		a_ind . i_s=0;
		gran_ring_char(&a_ind . s_i,0,30);
		}
...
#line 18601 "main.c"
	...
	if(but==251)
		{
		a_ind . s_i++;
		gran_char(&a_ind . s_i,0,11+NUMBAT+NUMIST +NUMMAKB +(NUMEXT!=0));
		...
		if(a_ind . s_i==0)
			...
			if(BAT_IS_ON[0]!=bisON)
				{
				if(BAT_TYPE==0)tree_up(iBat ,0,0,1);
				else if(BAT_TYPE==1) tree_up(iBat_li,0,0,1);
				}
		    	else 
				{
				if(BAT_TYPE==0)tree_up(iBat ,0,0,a_ind . s_i-1);
		...
		else if((a_ind . s_i>(NUMBAT+NUMIST  ))&&(a_ind . s_i<=(NUMBAT+NUMIST +NUMMAKB )))
		    	{
		    	tree_up(iMakb,0,0,a_ind . s_i-(1+NUMBAT+NUMIST ));
		    	}
		else if(a_ind . s_i==(1+NUMBAT+NUMIST +NUMMAKB ))
			{
		...
		else if(a_ind . s_i==(2+NUMBAT+NUMIST +NUMMAKB ))
			{
			tree_up(iLoad,0,0,0);
		     ret(1000);
			}
		else if((a_ind . s_i==(3+NUMBAT+NUMIST +NUMMAKB) )&&(NUMEXT))
			{
			tree_up(iExtern_6U,0,0,0);
		     ret(1000);
			}

		else if(a_ind . s_i==(3+NUMBAT+NUMIST +NUMMAKB +(NUMEXT!=0)))
			{
			tree_up(iSpc,0,0,0);
		     ret(1000);
			}

		else if(a_ind . s_i==(4+NUMBAT+NUMIST +NUMMAKB +(NUMEXT!=0)))
			{
			tree_up(iSet_prl,0,0,0);
		     ret(50);
		     parol_init();
			}
		else if(a_ind . s_i==(5+NUMBAT+NUMIST +NUMMAKB +(NUMEXT!=0)))
			{
			tree_up(iLog,0,0,0);
		     ret(1000);
			}
		else if(a_ind . s_i==(6+NUMBAT+NUMIST +NUMMAKB +(NUMEXT!=0)))
			{
			a_ind . s_i=0;
			}
		else if(a_ind . s_i==(7+NUMBAT+NUMIST +NUMMAKB +(NUMEXT!=0)))
			{
			tree_up(iBatLog,0,0,0);
		     ret(1000);
			}
		else if(a_ind . s_i==(8+NUMBAT+NUMIST +NUMMAKB +(NUMEXT!=0)))
			{
			tree_up(iBatLog,0,0,1);
		     ret(1000);
			}

		else if(a_ind . s_i==(9+NUMBAT+NUMIST +NUMMAKB +(NUMEXT!=0)))			{
			if(but==254)
		     	{
		...
		else if(a_ind . s_i==(10+NUMBAT+NUMIST +NUMMAKB +(NUMEXT!=0)))
			{
			if(but==254)
		     	{
		     	tree_up(iPrltst,0,0,0);
		...
		else if(a_ind . s_i==(11+NUMBAT+NUMIST +NUMMAKB +(NUMEXT!=0)))
			{
			if(but==254)
		     	{
	...
	if(but==251)
		{
		a_ind . s_i++;
		gran_char(&a_ind . s_i,0,10+NUMBAT+NUMIST+NUMINV+(NUMEXT!=0));
		}
	...
	if((num_rki>1)||(num_rki<0)) num_rki=0;
	if((num_net_in>3)||(num_rki<0)) num_net_in=0;
	if((NUMLVBD>1)||(NUMLVBD<0))NUMLVBD=0;
	if(but==251)
		{
		a_ind . s_i++;
		if((a_ind . s_i==(5+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0) ))	 && (SMART_SPC))
			{
			a_ind . i_s=a_ind . s_i+2;
			}
		else if((a_ind . s_i==(6+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0) )) && (SMART_SPC))
			{
			a_ind . s_i+=2;
			a_ind . i_s=a_ind . s_i+2;
			}
		else if((a_ind . s_i==(9+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0) )) && SMART_SPC)
			{
			a_ind . s_i+=2;
			
			}
		else if((a_ind . s_i==(10+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0) ))  && SMART_SPC)
			{
			
			}

		else if((a_ind . s_i==(9+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0))) && SMART_SPC) a_ind . s_i+=2;
		else if((a_ind . s_i==(10+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0))) && SMART_SPC) a_ind . s_i++;

		gran_char(&a_ind . s_i,0,12+NUMIST+(NUMEXT!=0)+(num_rki!=0)+(num_net_in!=0)+((SMART_SPC)*6)+NUMBAT+NUMLVBD); 
		}
	else if(but==253)
		{
		a_ind . s_i--;
  ...
  if((a_ind . s_i==(7+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0) )) && SMART_SPC)
			{
			a_ind . s_i-=2;
			a_ind . i_s=a_ind . s_i+2;
			}
		...
		{
		a_ind . s_i=(9+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0) + (6*(SMART_SPC!=0)));
		}
		...
		else if((a_ind . s_i==2)&&(NUMBAT))
		    	{
		    	tree_up(iBat_ZVU,0,0,0);
		    	} 
		else if((a_ind . s_i>1+NUMBAT)&&(a_ind . s_i<=(NUMIST+NUMBAT+1)))
		    	{
		    	tree_up(iBps_elteh,0,0,a_ind . s_i-2-NUMBAT);
		    	}
		else if(a_ind . s_i==(2+NUMIST+NUMBAT ))
			{
		...
		else if((a_ind . s_i==(3+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)))&&(NUMEXT))
			{
			tree_up(iExtern_220_ZVU,0,0,0);
		     ret(1000);
			}
		
	   	else if(a_ind . s_i==(3+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0)))
			{
			tree_up(iLVBD,0,0,0);
		     ret(1000);
			}
		...
		else if(a_ind . s_i==(3+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0)+NUMLVBD)) 
			{
			speedChargeStartStop();
			}

		else if(a_ind . s_i==(4+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0)+NUMLVBD)) 
			{
		...
		else if((a_ind . s_i==(5+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0)+NUMLVBD)) && (SMART_SPC)) 
			{
			if(but==254)
				{
				if(vz1_stat==vz1sOFF)
		...
		else if(a_ind . s_i==(2+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0) + (6*(SMART_SPC!=0))+NUMLVBD)) 
			{
			if(but==254)
				{
				if(vz2_stat==vz2sOFF)
		...
		else if(a_ind . s_i==(5+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0)+ (6*(SMART_SPC!=0))+NUMLVBD)) 
			{
			tree_up(iSet_prl,0,0,0);
		     ret(50);
		     parol_init();
			}
		else if(a_ind . s_i==(6+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0) + (6*(SMART_SPC!=0))+NUMLVBD)) 
			{
			if(but==126)avar_uout_hndl(1);
			tree_up(iLog,0,0,0);
		     ret(1000);
			}
		else if(a_ind . s_i==(7+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0) + (6*(SMART_SPC!=0))+NUMLVBD))  
			{
			a_ind . s_i=0;
			}
		...
		else if(a_ind . s_i==(8+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0) + (6*(SMART_SPC!=0))+NUMLVBD)) 
			{
			if(but==254)
		     	{
		     	tree_up(iPrltst,0,0,0);
		...
		else if(a_ind . s_i==(9+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0) + (6*(SMART_SPC!=0))+NUMLVBD)) 
			{
			if(but==254)
		     	{
		     	tree_up(iBps_list,0,0,0);
		...
		else if(a_ind . s_i==(10+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0) + (6*(SMART_SPC!=0))+NUMLVBD)) 
			{
			if(but==254)
		     	{
		     	tree_up(iFWabout,0,0,0);
		...
		else if(a_ind . s_i==(11+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0) + (6*(SMART_SPC!=0))+NUMLVBD)) 
			{
			if(but==254)
		     	{
		     	tree_up(iSpecInf,0,0,0);
		...
		else if(a_ind . s_i==(12+NUMBAT+NUMIST+(num_rki!=0)+(num_net_in!=0)+(NUMEXT!=0) + (6*(SMART_SPC!=0))+NUMLVBD)) 
			{
			if(but==254)
	...
	if(but==251)
		{
		a_ind . s_i++;
		gran_char(&a_ind . s_i,0,6+NUMIST);
		}
		...
		if(a_ind . s_i==0)
			...
			if(avar_ind_stat)
		    	...
		    	{
		    	tree_up(iBps,0,0,a_ind . s_i-1);
		    	}
		else if(a_ind . s_i==(1+NUMIST))
			{
...
#line 20515 "main.c"
	...
	if(but==251)
		...
		if(a_ind . s_i==0) command_rki=38;
		else if(a_ind . s_i==1) command_rki=42;
		...
		if(a_ind . s_i==0) command_rki=36;
		else if(a_ind . s_i==1) command_rki=40;
		...
		if(a_ind . s_i==0) command_rki=39;
		else if(a_ind . s_i==1) command_rki=43;
		else if(a_ind . s_i==3) command_rki=21;
		else if(a_ind . s_i==4) command_rki=25;
		else if(a_ind . s_i==5) command_rki=29;
		else if(a_ind . s_i==6) command_rki=17;	  
		if(type_rki==1){
		...
		if(a_ind . s_i==0) command_rki=37;
		else if(a_ind . s_i==1) command_rki=41;
		else if(a_ind . s_i==3) command_rki=22;
	...
	if(but==251)
		{
		a_ind . s_i++;
		if(type_rki==0) gran_char(&a_ind . s_i,0,12);
		else {
			if(count_Iddt==0) gran_char(&a_ind . s_i,0,39);
			else gran_char(&a_ind . s_i,0,39+(count_Iddt<<1) );
		}
		}
	else if(but==253)
		{
		...
		{
		if(type_rki==0) a_ind . s_i=12;
		else {
			if(count_Iddt==0) a_ind . s_i=39;
			else a_ind . s_i=39+(count_Iddt<<1);
		}
		}
	...
	if(but==251)
		{
		a_ind . s_i++;
		gran_char(&a_ind . s_i,0,1+lvbd_num_alarm_status);
		}
	else if(but==253)
		{
		a_ind . s_i--;
		...
		{
		a_ind . s_i=1+lvbd_num_alarm_status;
		}
	else if(but==125)	a_ind . s_i=0;	     
     }
	...
	if(but==251)
		...
		{
		a_ind . s_i--;
		if(a_ind . s_i==6) a_ind . s_i=5;
		gran_char(&a_ind . s_i,0,9);
		}
	else if(but==247){
		if(a_ind . s_i==0) command_lvbd=1;			
		else if(a_ind . s_i==1) command_lvbd=37;		
		else if(a_ind . s_i==2) command_lvbd=5;   	
		else if(a_ind . s_i==3) command_lvbd=9;   	
		else if(a_ind . s_i==4) command_lvbd=13;   	
		else if(a_ind . s_i==5) command_lvbd=17;   	
		else if(a_ind . s_i==7) command_lvbd=21;   	
		else if(a_ind . s_i==8) command_lvbd=25;   	
	}
	else if(but==119){
		if(a_ind . s_i==0) command_lvbd=2;
		else if(a_ind . s_i==1) command_lvbd=38;
		else if(a_ind . s_i==2) command_lvbd=6;
		else if(a_ind . s_i==3) command_lvbd=10;
		else if(a_ind . s_i==4) command_lvbd=14;
		else if(a_ind . s_i==5) command_lvbd=18;
		else if(a_ind . s_i==7) command_lvbd=22;
		else if(a_ind . s_i==8) command_lvbd=26;
	}
	else if(but==239){
		if(a_ind . s_i==0) command_lvbd=3;
		else if(a_ind . s_i==1) command_lvbd=39;
		else if(a_ind . s_i==2) command_lvbd=7;
		else if(a_ind . s_i==3) command_lvbd=11;
		else if(a_ind . s_i==4) command_lvbd=15;
		else if(a_ind . s_i==5) command_lvbd=19;
		else if(a_ind . s_i==7) command_lvbd=23;
		else if(a_ind . s_i==8) command_lvbd=27;
	}
	else if(but==111){
		if(a_ind . s_i==0) command_lvbd=4;
		else if(a_ind . s_i==1) command_lvbd=40;
		else if(a_ind . s_i==2) command_lvbd=8;
		else if(a_ind . s_i==3) command_lvbd=12;
		else if(a_ind . s_i==4) command_lvbd=16;
		else if(a_ind . s_i==5) command_lvbd=20;
		else if(a_ind . s_i==7) command_lvbd=24;
		else if(a_ind . s_i==8) command_lvbd=28;
	}
	else if(but==254)
	     {
		 	if( a_ind . s_i==9 ){
	...
	if(but==251)
		...
		if(a_ind . s_i==0) command_lvbd=29;			
		else if(a_ind . s_i==1) command_lvbd=33;   	
	}
	else if(but==119){
		if(a_ind . s_i==0) command_lvbd=30;
		else if(a_ind . s_i==1) command_lvbd=34;
	}
	else if(but==239){
		if(a_ind . s_i==0) command_lvbd=31;
		else if(a_ind . s_i==1) command_lvbd=35;
	}
	else if(but==111){
		if(a_ind . s_i==0) command_lvbd=32;
		else if(a_ind . s_i==1) command_lvbd=36;
	}
	else if(but==254)
	     {
		 	if( a_ind . s_i==2 ){
	...
	if(but==251)
		...
		if(a_ind . s_i<NUMENMV) {
			--enmv_modbus_adress[a_ind . s_i];
			lc640_write_int(4500+(a_ind . s_i*2),enmv_modbus_adress[a_ind . s_i]);
		}		
	}
	else if(but==119){
		if(a_ind . s_i<NUMENMV) {
			enmv_modbus_adress[a_ind . s_i]-=10;
			lc640_write_int(4500+(a_ind . s_i*2),enmv_modbus_adress[a_ind . s_i]);
		}
	}
	else if(but==239){
		...
		if(a_ind . s_i<NUMENMV) {
	     ...
	     {
		 	if(a_ind . s_i<NUMENMV) {
				enmv_modbus_adress[a_ind . s_i]+=50;
				lc640_write_int(4500+(a_ind . s_i*2),enmv_modbus_adress[a_ind . s_i]);
			}	
...
#line 21222 "main.c"
...
else if((a_ind . i==iNet3)||(a_ind . i==iNet3LIN))
	{
	ret(1000);
	ret_ind(0,0,0);
	if (but==253)
	...
	else if((but==247)||((but==254)&&(a_ind . s_i==5)))
	     {
	     tree_down(0,0);
	     ret(0);
	     }
...
#line 21739 "main.c"
	...
	||(a_ind . i==iPrltst)||(a_ind . i==iPrlVZ1)||(a_ind . i==iPrlVZ2)||(a_ind . i==iLog_reset_prl)
	||(a_ind . i==iPrl_Def_220_IPS_TERMOKOMPENSAT)||(a_ind . i==iSet_prl_FSO_inf))
	{
	ret(50);
	if(but==239)
...
#line 22171 "main.c"
		...
		else if(a_ind . i==iPrl_Def_220_IPS_TERMOKOMPENSAT)
			{
			if(tempU==295) 
				{
				char temp;
...
#line 22667 "main.c"
	...
	if (but==253)
		...
		{  
		if((a_ind . s_i==av_j_si_max+1)&&(av_j_si_max!=0))
			{
		...
		else if((a_ind . s_i==av_j_si_max)||(av_j_si_max==0))
			{
			tree_down(0,0);
			ret(0);
			}
...
#line 23261 "main.c"
	...
	if(but==251)
		...
		if(a_ind . s_i==3)a_ind . s_i=4;
		if(a_ind . s_i==7)a_ind . i_s=6;
		if(a_ind . s_i==8)a_ind . s_i=9;
		if(a_ind . s_i==11)a_ind . i_s=10;
		if(a_ind . s_i==12)a_ind . s_i=13;
		if(a_ind . s_i==33)a_ind . i_s=32;
		if(a_ind . s_i==34)a_ind . s_i=35;
		if(a_ind . s_i==35)a_ind . i_s=36;
		if(a_ind . s_i==36)a_ind . s_i=37;
		if(a_ind . s_i==41)a_ind . i_s=40;
		if(a_ind . s_i==42)a_ind . s_i=43;
		if(a_ind . s_i==43)	a_ind . i_s=42;
        if(a_ind . s_i==44)	a_ind . s_i=45;
        if(a_ind . s_i==46)a_ind . i_s=45;
        if(a_ind . s_i==47)a_ind . s_i=48;
		gran_char(&a_ind . s_i,0,51);
		}
	else if(but==253)
		{
		a_ind . s_i--;
		if(a_ind . s_i==3)a_ind . s_i=2;
		if(a_ind . s_i==8)a_ind . s_i=7;
		if(a_ind . s_i==12)a_ind . s_i=9;
		if(a_ind . s_i==42)a_ind . s_i=41;
		if(a_ind . s_i==41)a_ind . i_s=41;
        if(a_ind . s_i==36)a_ind . s_i=35;
        if(a_ind . s_i==35)a_ind . i_s=35;
        if(a_ind . s_i==34)a_ind . s_i=33;
        if(a_ind . s_i==33)a_ind . i_s=33;
        if(a_ind . s_i==44)a_ind . s_i=43;
        if(a_ind . s_i==47)a_ind . s_i=46;
		gran_char(&a_ind . s_i,0,51);
		}
	else if(but==123)
		{
		...
		gran(&USIGN,15,100);
	     ...
	     {
	     if(but==239)UMAXN++;
	     else if(but==111)UMAXN+=10;
	     else if(but==247)UMAXN--;
	     else if(but==119)UMAXN-=10;
	     gran(&UMAXN,1,300);
	     lc640_write_int(4470,UMAXN);
	     speed=1;
	     }
		...
		if(NUMENMV) 
			{ 
			tree_up(iSetENMV,0,0,0); 
			ret(1000);
			}
		}
	
     else if((a_ind . s_i==50) || (a_ind . s_i==5))	 
		{
		if(but==254)
		     {
		     tree_down(0,0);
...
#line 27262 "main.c"
	...
	if(but==251)
		...
		if(a_ind . s_i==8)a_ind . i_s=7;
		if(a_ind . s_i==9)a_ind . s_i=10;
		if(a_ind . s_i==22)a_ind . i_s=21;
		if(a_ind . s_i==23)a_ind . s_i=24;
		if(a_ind . s_i==25)a_ind . i_s=24;
		if(a_ind . s_i==26)a_ind . s_i=27;
		if(a_ind . s_i==27)a_ind . i_s=26;
		if(a_ind . s_i==28)a_ind . s_i=29;
		if(a_ind . s_i==33)a_ind . i_s=32;
		if(a_ind . s_i==34)a_ind . s_i=35;
		if(a_ind . s_i==36)a_ind . i_s=35;
		if(a_ind . s_i==37)a_ind . s_i=38;
		if(a_ind . s_i==40)a_ind . i_s=39;
		if(a_ind . s_i==41)a_ind . s_i=42;
		if(a_ind . s_i==42)a_ind . i_s=41;
		if(a_ind . s_i==43)a_ind . s_i=44;
		if(a_ind . s_i==49)a_ind . i_s=48;
		if(a_ind . s_i==50)a_ind . s_i=51; 
		...
		if(a_ind . s_i==41)a_ind . s_i=40;
		if(a_ind . s_i==43)a_ind . s_i=42;
		if(a_ind . s_i==50)a_ind . s_i=49; 
		gran_char(&a_ind . s_i,0,52); 
		...
		lc640_write_int(0x10+100+226,20000);
	     ...
	     if(but==254)
	          {
	          tree_up(iPrl_Def_220_IPS_TERMOKOMPENSAT,0,0,0);
			  parol_init();
			  ret(50);
	          }
	     }	
		...
		lc640_write_int(0x10+350+56,SMART_SPC);
		speed=1;
		}

	else if((a_ind . s_i==51) || (a_ind . s_i==6)) 
		{
		if(but==254)
		     {
		     tree_down(0,0);
	...
	if(but==251)
		...
		gran(&UB0,100,3000);
		lc640_write_int(0x10+100+6,UB0);
		lc640_write_int(0x10+100+8,UB0);
		speed=1;
		}
	...
	else if((a_ind . s_i==26) || (a_ind . s_i==4))
		{
		if(but==254)
		     {
		     tree_down(0,0);
...
#line 30135 "main.c"
	...
	if(but==251)
		...
		if(a_ind . s_i==0)
			...
			{
			def_set(600,564,545,44,150,480,4,576);
			lc640_write_int(0x10+100+224,48123);
			}
	...
	if(but==251)
		...
		if(a_ind . s_i==0)
			...
			lc640_write_int(0x10+100+224,22044);
			lc640_write_int(0x10+100+36,3);
	...
	if(but==251)
		{
		a_ind . s_i++;
		if(SNTP_ENABLE==0)gran_char(&a_ind . s_i,0,1);
		else gran_char(&a_ind . s_i,0,5);
		if(a_ind . s_i==2)
			{
			a_ind . i_s=2;
			}
		...
		if(a_ind . s_i==3)
	     ...
	     if((but==239)||(but==111))
	     	...
	     	lc640_write_int(0x10+350+26,SNTP_ENABLE);
	     	}
		...
		if(SNTP_ENABLE==0)
		...
		else 
			{
	     	if((but==239)||(but==111))
	     		...
	     		SNTP_GMT++;
	     		gran(&SNTP_GMT,-12,13);
	     		lc640_write_int(0x10+350+28,SNTP_GMT);
	     		}
		...
		if(but==254)
	     	{
	     	if(lc640_read_int(4472)==1)lc640_write_int(4472,0);
			...
			if((but==239)||(but==111))
				{
				_temp=lc640_read_int(4474);
				_temp++;
				gran_ring(&_temp,0,255);
				lc640_write_int(4474,_temp);
				}
			else if((but==247)||(but==119))
				{
				_temp=lc640_read_int(4474);
			...
			if((but==239)||(but==111))
				{
				_temp=lc640_read_int(4476);
				_temp++;
				gran_ring(&_temp,0,255);
				lc640_write_int(4476,_temp);
				}
			else if((but==247)||(but==119))
				{
				_temp=lc640_read_int(4476);
			...
			if((but==239)||(but==111))
				{
				_temp=lc640_read_int(4478);
				_temp++;
				gran_ring(&_temp,0,255);
				lc640_write_int(4478,_temp);
				}
			else if((but==247)||(but==119))
				{
				_temp=lc640_read_int(4478);
			...
			if((but==239)||(but==111))
				{
				_temp=lc640_read_int(4480);
				_temp++;
				gran_ring(&_temp,0,255);
				lc640_write_int(4480,_temp);
				}
			else if((but==247)||(but==119))
				{
				_temp=lc640_read_int(4480);
		...
		if(but==126)
			...
			if(lc640_read_int(4472)==1)
				...
				{
				Rem_IP[0]=(char)lc640_read_int(4474);
				Rem_IP[1]=(char)lc640_read_int(4476);
				Rem_IP[2]=(char)lc640_read_int(4478);
				Rem_IP[3]=(char)lc640_read_int(4480);
				}
			sntp_requ();
	        }
		}	
	...
	if(but==251)
		...
		a_ind . s_i++;
		if(a_ind . s_i==4)a_ind . i_s=3;
		if(a_ind . s_i==5)a_ind . s_i++;
		gran_char(&a_ind . s_i,1,9);
		}
	else if(but==253)
		{
		a_ind . s_i--;
		...
		if((but==239)||(but==111))
			{
	     	...
	     	lc640_write_int(4352,NUMENMV);
	     	}
	     
		else if((but==247)||(but==119))
	     	{
	     	if(NUMENMV==0) NUMENMV=8; 
			else --NUMENMV;	
	     	lc640_write_int(4352,NUMENMV);
	     	}
		}
	     ...
	     if((but==239)||(but==111))
	     	...
	     	NUMBDR++;
	     	gran(&NUMBDR,0,1);
	     	lc640_write_int(0x10+500+104,NUMBDR);
	     	}
...
#line 31961 "main.c"
	...
	if(but==251)
     ...
     else if(a_ind . s_i==0)
		{
	     if((but==239)||(but==111))
	     	...
	     	NUMBAT++;
	     	gran(&NUMBAT,0,1);
	     	lc640_write_int(0x10+350+72,NUMBAT);
		...
		if((but==239)||(but==111))
			{
	     	if(NUMLVBD) NUMLVBD=0;
			else NUMLVBD=1;
	     	lc640_write_int(4356,NUMLVBD);
	     	}
	...
	if(but==251)
		...
		if(but==126)
	     	{
	     	a_ind . s_i1++;
			gran_ring_char(&a_ind . s_i1,0,2);
	     	}
		if((but==239)||(but==111))
			{
			snmp_web_passw[a_ind . s_i1]++;
			if(snmp_web_passw[a_ind . s_i1]<32) snmp_web_passw[a_ind . s_i1]=32;
			else if ((snmp_web_passw[a_ind . s_i1]>32)&&(snmp_web_passw[a_ind . s_i1]<48)) snmp_web_passw[a_ind . s_i1]=48;
			else if ((snmp_web_passw[a_ind . s_i1]>57)&&(snmp_web_passw[a_ind . s_i1]<65)) snmp_web_passw[a_ind . s_i1]=65;
			else if ((snmp_web_passw[a_ind . s_i1]>90)&&(snmp_web_passw[a_ind . s_i1]<97)) snmp_web_passw[a_ind . s_i1]=97;
			else if (snmp_web_passw[a_ind . s_i1]>122) snmp_web_passw[a_ind . s_i1]=32;
				
			lc640_write_int(4360+(a_ind . s_i1*2),snmp_web_passw[a_ind . s_i1]);
			speed=1;
			}
		if((but==247)||(but==119))
			{
			snmp_web_passw[a_ind . s_i1]--;
			if(snmp_web_passw[a_ind . s_i1]<32) snmp_web_passw[a_ind . s_i1]=122;
			else if ((snmp_web_passw[a_ind . s_i1]>32)&&(snmp_web_passw[a_ind . s_i1]<48)) snmp_web_passw[a_ind . s_i1]=32;
			else if ((snmp_web_passw[a_ind . s_i1]>57)&&(snmp_web_passw[a_ind . s_i1]<65)) snmp_web_passw[a_ind . s_i1]=57;
			else if ((snmp_web_passw[a_ind . s_i1]>90)&&(snmp_web_passw[a_ind . s_i1]<97)) snmp_web_passw[a_ind . s_i1]=90;
			else if (snmp_web_passw[a_ind . s_i1]>122) snmp_web_passw[a_ind . s_i1]=122;
	...
	if(but==251)
		...
		if((but==239)||(but==111))
			{
			SP_CH_VENT_BLOK=1;
			lc640_write_int(0x10+350+96,SP_CH_VENT_BLOK);
			}
		else if((but==247)||(but==119))
			{
			SP_CH_VENT_BLOK=0;
...
#line 34590 "main.c"
	...
	if(but==251)
		{
		a_ind . s_i++;
		gran_char(&a_ind . s_i,0,4+(NUMIST!=0)+(NUMDT!=0)+(num_rki!=0)+(num_net_in!=0)+(NUMLVBD!=0)); 
		}
	else if(but==253)
		{
		a_ind . s_i--;
		...
		{
		a_ind . s_i=3+(NUMIST!=0)+(NUMDT!=0)+(num_rki!=0)+(num_net_in!=0)+(NUMLVBD!=0);
		}
	else if(a_ind . s_i==(4+(NUMIST!=0)+(NUMDT!=0)+(num_rki!=0)+(num_net_in!=0)+(NUMLVBD!=0))) 
			{
			if((but==239)||(but==111))
				{
				if(RS485_QWARZ_DIGIT==10)RS485_QWARZ_DIGIT=30;
		...
		else if(NUMLVBD && (a_ind . s_i==(3+(NUMIST!=0)+(NUMDT!=0)+(num_rki!=0)+(num_net_in!=0) ) ))
			{
			tree_up(iK_LVBD,0,0,0);	
			ret(1000);			
			}
 		
        else if(a_ind . s_i==(3+(NUMIST!=0)+(NUMDT!=0)+(num_rki!=0)+(num_net_in!=0)+(NUMLVBD!=0))) 
			{
	          tree_down(0,0);
	          ret(0);
            }
	...
	if(but==251)
		{
		a_ind . s_i++;
		gran_char(&a_ind . s_i,0,3+(NUMIST!=0));
		}
	else if(but==253)
		{
		a_ind . s_i--;
		...
		else if((NUMIST)&&(a_ind . s_i==1))
			{
			tree_up(iK_bps_sel,0,0,0);	
			ret(1000);
			}

		else if((a_ind . s_i==(1+(NUMIST!=0))))
			{
			tree_up(iK_load,0,0,0);	
			ret(1000);
			}
	...
	if(but==251)
		{
		a_ind . s_i++;
		gran_char(&a_ind . s_i,0,NUMBAT_FSO);
		phase=0;
		}
	else if(but==253)
		{
		...
		{
		a_ind . s_i=1+NUMBAT_FSO;
		}
	else if(a_ind . s_i==0)
		{
		if((but==254)&&(NUMBAT_FSO>0))
			{
			tree_up(iK_bat_TELECORE,0,0,0);	
			ret(1000);
			}
		}	
	else if((a_ind . s_i==1)&&(NUMBAT_FSO>1))
		{
		if((but==254)&&(NUMBAT_FSO>1))
			{
			tree_up(iK_bat_TELECORE,0,0,1);	
			ret(1000);
			}
		}
	else if(a_ind . s_i==(NUMBAT_FSO))
		{
		if(but==254)
			{
			tree_down(0,0);
	...
	if(but==251)
		...
		if(but==239)temp_SS++;
		else if(but==111)temp_SS+=10;
		else if(but==247)temp_SS--;
		else if(but==119)temp_SS-=10;
...
#line 36839 "main.c"
 ...
 tree_down(0,0);	
	...
	if(but==251)
		{
		a_ind . s_i++;
		gran_char(&a_ind . s_i,0,14+NUMIST+NUMBDR*8);	
		phase=0;
		tst_state[0]=tstOFF;
		tst_state[1]=tstOFF;
		tst_state[2]=tstOFF;
		...
		if((a_ind . s_i==(13+NUMIST))&&(NUMBDR==1))
			{
			a_ind . s_i=(13+NUMIST);
			a_ind . i_s=(12+NUMIST);
			}
		if((a_ind . s_i==(14+NUMIST))&&(NUMBDR==1))
			{
			a_ind . s_i=(15+NUMIST);
			a_ind . i_s=(14+NUMIST);
			}
		if((a_ind . s_i==(16+NUMIST))&&(NUMBDR==1))
			{
			a_ind . s_i=(17+NUMIST);
			a_ind . i_s=(16+NUMIST);
			}
		if((a_ind . s_i==(18+NUMIST))&&(NUMBDR==1))
			{
			a_ind . s_i=(19+NUMIST);
			a_ind . i_s=(18+NUMIST);
			} 
		if((a_ind . s_i==(20+NUMIST))&&(NUMBDR==1))
			{
			a_ind . s_i=(21+NUMIST);
			a_ind . i_s=(19+NUMIST);
			}
		...
		if((but==254)||(but==239))
			{
			if(tst_state[10]==tstOFF) tst_state[10]=tst1;
			else if(tst_state[10]==tst1) tst_state[10]=tst2;
			else tst_state[10]=tstOFF;
			}
		else if(but==247)
			{
			if(tst_state[10]==tst2) tst_state[10]=tst1;
			else if(tst_state[10]==tstOFF) tst_state[10]=tst2;
			else tst_state[10]=tstOFF;
			}
		}
		...
		if((but==254)||(but==239))
			{
			if(tst_state[11]==tstOFF) tst_state[11]=tst1;
			else if(tst_state[11]==tst1) tst_state[11]=tst2;
			else tst_state[11]=tstOFF;
			}
		else if(but==247)
			{
			if(tst_state[11]==tst2) tst_state[11]=tst1;
			else if(tst_state[11]==tstOFF) tst_state[11]=tst2;
			else tst_state[11]=tstOFF;
			}
		}
		...
		if((but==254)||(but==239))
			{
			if(tst_state[12]==tstOFF) tst_state[12]=tst1;
			else if(tst_state[12]==tst1) tst_state[12]=tst2;
			else tst_state[12]=tstOFF;
			}
		else if(but==247)
			{
			if(tst_state[12]==tst2) tst_state[12]=tst1;
			else if(tst_state[12]==tstOFF) tst_state[12]=tst2;
			else tst_state[12]=tstOFF;
			}
		}
		...
		if((but==254)||(but==239))
			{
			if(tst_state[13]==tstOFF) tst_state[13]=tst1;
			else if(tst_state[13]==tst1) tst_state[13]=tst2;
			else tst_state[13]=tstOFF;
			}
		else if(but==247)
			{
			if(tst_state[13]==tst2) tst_state[13]=tst1;
			else if(tst_state[13]==tstOFF) tst_state[13]=tst2;
			else tst_state[13]=tstOFF;
			}
		}

													
	else if(a_ind . s_i==(13+NUMIST+NUMBDR*8))
		{
		if(but==254)
			{
			bRESET=1;
	...
	else if(a_ind . s_i==(14+NUMIST+NUMBDR*8)) 
		{
		if(but==254)
			{
			tree_down(0,0);
	...
	if(but==251)
		{
		a_ind . s_i++;
		gran_char(&a_ind . s_i,0,10+NUMIST + 8*(NUMBDR==1));
		phase=0;
		tst_state[0]=tstOFF;
		tst_state[1]=tstOFF;
		tst_state[2]=tstOFF;
		...
		if(a_ind . s_i==(8+NUMIST))
			{
			a_ind . i_s=(7+NUMIST);
			}

		if((a_ind . s_i==(9+NUMIST))&&(NUMBDR==1))
			{
			a_ind . s_i=(10+NUMIST);
			a_ind . i_s=(9+NUMIST);
			}
		if((a_ind . s_i==(11+NUMIST))&&(NUMBDR==1))
			{
			a_ind . s_i=(12+NUMIST);
			a_ind . i_s=(11+NUMIST);
			}
		...
		if((a_ind . s_i==(15+NUMIST))&&(NUMBDR==1))
			{
			a_ind . s_i=(16+NUMIST);
			a_ind . i_s=(15+NUMIST);
			}
	...
	else if(a_ind . s_i==(8+NUMIST+(8*(NUMBDR==1))))
		{
		if(but==254)
			{
			tree_down(0,0);
	...
	else if(a_ind . s_i==(9+NUMIST+(8*(NUMBDR==1))))
		{
		if(but==254)
			{
			bRESET_INT_WDT=1;
	...
	else if(a_ind . s_i==(10+NUMIST+(8*(NUMBDR==1))))
		{
		if(but==254)
			{
			bRESET_EXT_WDT=1;
...
#line 41358 "main.c"
	...
	if(but==251)
		...
		else if (NPN_OUT==npnoRELEAVBAT2) NPN_OUT=npnoBDR;	
		else if(NPN_OUT==npnoBDR)NPN_OUT=npnoOFF;		   	
	...
	if(but==251)
	...
	else if((but==254)&&(a_ind . s_i>=0)&&(a_ind . s_i<=3))
		{
		...
		tree_up(iRele_set_,0,0,a_ind . s_i);
	...
	if(but==251)
		...
		a_ind . s_i--;
		if(a_ind . s_i==3)a_ind . s_i=2;
		else if(a_ind . s_i==8)a_ind . s_i=7;
		else if(a_ind . s_i==11)a_ind . s_i=10;
		gran_char(&a_ind . s_i,0,12);		
		}
	else if(but==123)
		{
		...
		{
		if(but==254) RELE_SET_MASK[a_ind . s_i1]^=(1<<0);
	    else if(but==239) RELE_SET_MASK[a_ind . s_i1]|=(1<<0);
    	else if(but==247) RELE_SET_MASK[a_ind . s_i1]&=~(1<<0);
		}	
		
	else if (a_ind . s_i == 1)
		{
		if(but==254) RELE_SET_MASK[a_ind . s_i1]^=(1<<1);
	    else if(but==239) RELE_SET_MASK[a_ind . s_i1]|=(1<<1);
    	else if(but==247) RELE_SET_MASK[a_ind . s_i1]&=~(1<<1);
		}	
		
	else if (a_ind . s_i == 2)
		{
		if(but==254) RELE_SET_MASK[a_ind . s_i1]^=(1<<2);
	    else if(but==239) RELE_SET_MASK[a_ind . s_i1]|=(1<<2);
    	else if(but==247) RELE_SET_MASK[a_ind . s_i1]&=~(1<<2);
		}
		
	else if (a_ind . s_i == 4)
		{
		if(but==254) RELE_SET_MASK[a_ind . s_i1]^=(1<<3);
	    else if(but==239) RELE_SET_MASK[a_ind . s_i1]|=(1<<3);
    	else if(but==247) RELE_SET_MASK[a_ind . s_i1]&=~(1<<3);
		}	
		
	else if (a_ind . s_i == 5)
		{
		if(but==254) RELE_SET_MASK[a_ind . s_i1]^=(1<<4);
	    else if(but==239) RELE_SET_MASK[a_ind . s_i1]|=(1<<4);
    	else if(but==247) RELE_SET_MASK[a_ind . s_i1]&=~(1<<4);
		}							

	else if (a_ind . s_i == 6)
		{
		if(but==254) RELE_SET_MASK[a_ind . s_i1]^=(1<<5);
	    else if(but==239) RELE_SET_MASK[a_ind . s_i1]|=(1<<5);
    	else if(but==247) RELE_SET_MASK[a_ind . s_i1]&=~(1<<5);
		}
	else if (a_ind . s_i == 7)
		{
		if(but==254) RELE_SET_MASK[a_ind . s_i1]^=(1<<6);
	    else if(but==239) RELE_SET_MASK[a_ind . s_i1]|=(1<<6);
    	else if(but==247) RELE_SET_MASK[a_ind . s_i1]&=~(1<<6);
		}
	else if (a_ind . s_i == 9)
		{
		if(but==254) RELE_SET_MASK[a_ind . s_i1]^=(1<<7);
	    else if(but==239) RELE_SET_MASK[a_ind . s_i1]|=(1<<7);
    	else if(but==247) RELE_SET_MASK[a_ind . s_i1]&=~(1<<7);
		}

	else if (a_ind . s_i == 10)
		{
		if(but==254) RELE_SET_MASK[a_ind . s_i1]^=(1<<15);
	    else if(but==239) RELE_SET_MASK[a_ind . s_i1]|=(1<<15);
    	else if(but==247) RELE_SET_MASK[a_ind . s_i1]&=~(1<<15);
		} 
	...
	if(RELE_SET_MASK[a_ind . s_i1]!=lc640_read_int(ADR_EE_RELE_SET_MASK[a_ind . s_i1]))lc640_write_int(ADR_EE_RELE_SET_MASK[a_ind . s_i1],RELE_SET_MASK[a_ind . s_i1]);
	...
	if(but==251)
		...
		a_ind . s_i--;
		if(a_ind . s_i==2)a_ind . s_i=1;
		else if(a_ind . s_i==9)a_ind . s_i=8;
		gran_char(&a_ind . s_i,0,10);		
		}
	else if(but==123)
		{
	...
	else if(a_ind . s_i==simax-1)
		{
		if(but==126)
			{
			lc640_write_int(0x10+350+90,0);
			}
		}
	}
else if(a_ind . i==iSpecInfFSO)
 ...
 if(a_ind . s_i==0)
	    ...
	    {
		if(but==239)IMAX_VZ++;
	    else if(but==111)IMAX_VZ+=2;
	    else if(but==247)IMAX_VZ--;
	    else if(but==119)IMAX_VZ-=2;
		gran(&IMAX_VZ,10,2000); 	          
		lc640_write_int(0x10+350+54,IMAX_VZ);
	    speed=1;
	    }
	else if(a_ind . s_i==2)
		{
		...
		if((but==239)||(but==111))
			{
			VZ_CH_VENT_BLOK=1;
			lc640_write_int(0x10+350+98,VZ_CH_VENT_BLOK);
			}
		else if((but==247)||(but==119))
			{
			VZ_CH_VENT_BLOK=0;
	...
	if(but==251)
	    ...
	    {
		if(but==239)UZ_U++;
	    else if(but==111)UZ_U+=2;
	    else if(but==247)UZ_U--;
	    else if(but==119)UZ_U-=2;
		gran(&UZ_U,UB20,6000); 	          
		lc640_write_int(0x10+350+30,UZ_U);
	    speed=1;
	    }
	    ...
	    {
		if(but==239)UZ_IMAX++;
	    else if(but==111)UZ_IMAX+=10;
	    else if(but==247)UZ_IMAX--;
	    else if(but==119)UZ_IMAX-=10;
		gran(&UZ_IMAX,10,10000); 	          
		lc640_write_int(0x10+350+32,UZ_IMAX);
	    speed=1;
	    }
	    ...
	    {
		if(but==239)UZ_T++;
	    else if(but==111)UZ_T++;
	    else if(but==247)UZ_T--;
	    else if(but==119)UZ_T--;
		gran(&UZ_T,1,72); 	          
		lc640_write_int(0x10+350+34,UZ_T);
	    speed=1;
	    }
	else if(a_ind . s_i==3)
		{
	...
	if(but==251)
	    ...
	    {
		if(but==239)FZ_U1++;
	    else if(but==111)FZ_U1+=2;
	    else if(but==247)FZ_U1--;
	    else if(but==119)FZ_U1-=2;
		gran(&FZ_U1,UB20,6000); 	          
		lc640_write_int(0x10+350+40,FZ_U1);
	    speed=1;
	    }
	    ...
	    {
		if(but==239)FZ_IMAX1++;
	    else if(but==111)FZ_IMAX1+=2;
	    else if(but==247)FZ_IMAX1--;
	    else if(but==119)FZ_IMAX1-=2;
		gran(&FZ_IMAX1,10,1000); 	          
		lc640_write_int(0x10+350+42,FZ_IMAX1);
	    speed=1;
	    }
	else if(a_ind . s_i==2)
	    {
		if(but==239)FZ_T1++;
	    else if(but==111)FZ_T1++;
	    else if(but==247)FZ_T1--;
	    else if(but==119)FZ_T1--;
		gran(&FZ_T1,1,10); 	          
		lc640_write_int(0x10+350+44,FZ_T1);
	    speed=1;
	    }
	else if(a_ind . s_i==3)
	    {
		if(but==239)FZ_ISW12++;
	    else if(but==111)FZ_ISW12+=2;
	    else if(but==247)FZ_ISW12--;
	    else if(but==119)FZ_ISW12-=2;
		gran(&FZ_ISW12,10,1000); 	          
		lc640_write_int(0x10+350+46,FZ_ISW12);
	    speed=1;
	    }
	else if(a_ind . s_i==4)
	    {
		if(but==239)FZ_U2++;
	    else if(but==111)FZ_U2+=2;
	    else if(but==247)FZ_U2--;
	    else if(but==119)FZ_U2-=2;
		gran(&FZ_U2,UB20,6000); 	          
		lc640_write_int(0x10+350+48,FZ_U2);
	    speed=1;
	    }
	    ...
	    {
		if(but==239)FZ_IMAX2++;
	    else if(but==111)FZ_IMAX2+=2;
	    else if(but==247)FZ_IMAX2--;
	    else if(but==119)FZ_IMAX2-=2;
		gran(&FZ_IMAX2,10,1000); 	          
		lc640_write_int(0x10+350+50,FZ_IMAX2);
	    speed=1;
	    }
	else if(a_ind . s_i==6)
	    {
		if(but==239)FZ_T2++;
	    else if(but==111)FZ_T2++;
	    else if(but==247)FZ_T2--;
	    else if(but==119)FZ_T2--;
		gran(&FZ_T2,1,10); 	          
		lc640_write_int(0x10+350+52,FZ_T2);
	    speed=1;
	    }
	else if(a_ind . s_i==7)
		{
	...
	if(but==251)
		...
		if(a_ind . s_i==0)
			{
			hv_vz_stat=hvsSTEP3;
			lc640_write(0x10+100+244,hvsSTEP3);
			}
		else if(a_ind . s_i==1)
			{
 			hv_vz_stat=hvsOFF;
	...
	if(but==251)
		...
		if(a_ind . s_i==0)
			{
			vz1_stat=vz1sSTEP3;
			lc640_write(0x10+350+36,vz1sSTEP3);
			}
		else if(a_ind . s_i==1)
			{
 			vz1_stat==vz1sOFF;
	...
	if(but==251)
		...
		if(a_ind . s_i==0)
			{
			vz2_stat=vz2sSTEP3;
			lc640_write(0x10+350+38,vz2sSTEP3);
			}
		else if(a_ind . s_i==1)
			{
 			vz2_stat==vz2sOFF;
	...
	if(but==251)
		...
		{
		if(but==239)dUNPN++;
	    else if(but==111)dUNPN+=2;
	    else if(but==247)dUNPN--;
	    else if(but==119)dUNPN-=2;
	    gran(&dUNPN,1,250);
	    lc640_write_int(0x10+100+248,dUNPN);
	    speed=1;
		}
	else if(a_ind . s_i==3)
		{
	...
	if(but==253)
		...
		if(a_ind . s_i==22)a_ind . s_i--;
		if((a_ind . s_i==29)||(a_ind . s_i==30))a_ind . s_i=28;
		if((a_ind . s_i==32)||(a_ind . s_i==33))a_ind . s_i=31;
		if(a_ind . s_i==35)a_ind . s_i--;
		}
	else if (but==251)
		{
		a_ind . s_i++;
		gran_char(&a_ind . s_i,0,36);
		if(a_ind . s_i==1)a_ind . s_i++;
		if(a_ind . s_i==21)a_ind . i_s=20;
		if(a_ind . s_i==22)a_ind . s_i++;
		if(a_ind . s_i==28)a_ind . i_s=30;
		if((a_ind . s_i==29)||(a_ind . s_i==30))a_ind . s_i=31;
        if(a_ind . s_i==31)a_ind . i_s=33;
		if((a_ind . s_i==32)||(a_ind . s_i==33))a_ind . s_i=34;
		if(a_ind . s_i==34)a_ind . i_s=33;
		if(a_ind . s_i==35)a_ind . s_i++;
		...
		if(but==254)
	     ...
	     lc640_write_int(0x10+350+92,BAT_C_POINT_NUM_ELEM);
	     speed=1;
		 if(bI)
		 	{
			bI=0;
			lc640_write_int(0x10+100+6,(short)((long)BAT_C_POINT_NUM_ELEM*237L/10L));
			lc640_write_int(0x10+100+8,(short)((long)BAT_C_POINT_NUM_ELEM*227L/10L));
			lc640_write_int(0x10+100+14,(short)((long)BAT_C_POINT_NUM_ELEM*180L/100L));
			}
	     }
	else if(a_ind . s_i==2)
	     {
	     if(but==239)BAT_C_POINT_20++;
	     else if(but==111)BAT_C_POINT_20=(BAT_C_POINT_20/10+1)*10;
	     else if(but==247)BAT_C_POINT_20--;
	     else if(but==119)BAT_C_POINT_20=(BAT_C_POINT_20/10-1)*10;
	     gran(&BAT_C_POINT_20,10,25000);
	     lc640_write_int(0x10+350+70,BAT_C_POINT_20);
		 speed=1;
		 if(but==126)
		 	{
			BAT_C_POINT_10=(signed short)((((long)BAT_C_POINT_20)*95L)/100L);
			lc640_write_int(0x10+350+68,BAT_C_POINT_10);
			BAT_C_POINT_5=(signed short)((((long)BAT_C_POINT_20)*9L)/10L);
			lc640_write_int(0x10+350+66,BAT_C_POINT_5);
			BAT_C_POINT_3=(signed short)((((long)BAT_C_POINT_20)*8L)/10L);
			lc640_write_int(0x10+350+64,BAT_C_POINT_3);
			BAT_C_POINT_1=(signed short)((((long)BAT_C_POINT_20)*6L)/10L);
			lc640_write_int(0x10+350+62,BAT_C_POINT_1);
			BAT_C_POINT_1_2=(signed short)((((long)BAT_C_POINT_20)*5L)/10L);
			lc640_write_int(0x10+350+60,BAT_C_POINT_1_2);
			BAT_C_POINT_1_6=(signed short)((((long)BAT_C_POINT_20)*3L)/10L);
			lc640_write_int(0x10+350+58,BAT_C_POINT_1_6);
			}
	     }
	else if(a_ind . s_i==3)
	     {
	     if(but==239)BAT_U_END_20++;
	     else if(but==111)BAT_U_END_20=(BAT_U_END_20/10+1)*10;
	     else if(but==247)BAT_U_END_20--;
	     else if(but==119)BAT_U_END_20=(BAT_U_END_20/10-1)*10;
	     gran(&BAT_U_END_20,10,10000);
	     lc640_write_int(0x10+350+86,BAT_U_END_20);
		 speed=1;
		 if(but==126)
		 	{
			BAT_U_END_10=(signed short)((((long)BAT_U_END_20)*97L)/100L);
			lc640_write_int(0x10+350+84,BAT_U_END_10);
			BAT_U_END_5=(signed short)((((long)BAT_U_END_20)*94L)/100L);
			lc640_write_int(0x10+350+82,BAT_U_END_5);			
			BAT_U_END_3=(signed short)((((long)BAT_U_END_20)*93L)/100L);
			lc640_write_int(0x10+350+80,BAT_U_END_3);
			BAT_U_END_1=(signed short)((((long)BAT_U_END_20)*92L)/100L);
			lc640_write_int(0x10+350+78,BAT_U_END_1);
			BAT_U_END_1_2=(signed short)((((long)BAT_U_END_20)*91L)/100L);
			lc640_write_int(0x10+350+76,BAT_U_END_1_2);
			BAT_U_END_1_6=(signed short)((((long)BAT_U_END_20)*89L)/100L);
			lc640_write_int(0x10+350+74,BAT_U_END_1_6);
			}
	     }
	else if(a_ind . s_i==4)
	     {
		 ...
		 if(bI)
		 	{
			bI=0;
			lc640_write_int(0x10+100+30,(short)((long)BAT_C_POINT_10/10L));
			}
		 if(but==126)
		 	{
		...
		{
		if(but==239)BAT_U_END_10++;
		else if(but==111)BAT_U_END_10=(BAT_U_END_10/10+1)*10;
		else if(but==247)BAT_U_END_10--;
		else if(but==119)BAT_U_END_10=(BAT_U_END_10/10-1)*10;
		...
		if(but==126)
	     ...
	     {
	     if(but==239)BAT_C_POINT_5++;
	     else if(but==111)BAT_C_POINT_5=(BAT_C_POINT_5/10+1)*10;
	     else if(but==247)BAT_C_POINT_5--;
	     else if(but==119)BAT_C_POINT_5=(BAT_C_POINT_5/10-1)*10;
	     ...
	     {
	     if(but==239)BAT_U_END_5++;
	     else if(but==111)BAT_U_END_5=(BAT_U_END_5/10+1)*10;
	     else if(but==247)BAT_U_END_5--;
	     else if(but==119)BAT_U_END_5=(BAT_U_END_5/10-1)*10;
	     ...
	     {
	     if(but==239)BAT_C_POINT_3++;
	     else if(but==111)BAT_C_POINT_3=(BAT_C_POINT_3/10+1)*10;
	     else if(but==247)BAT_C_POINT_3--;
	     else if(but==119)BAT_C_POINT_3=(BAT_C_POINT_3/10-1)*10;
		...
		{
		if(but==239)BAT_U_END_3++;
		else if(but==111)BAT_U_END_3=(BAT_U_END_3/10+1)*10;
		else if(but==247)BAT_U_END_3--;
		else if(but==119)BAT_U_END_3=(BAT_U_END_3/10-1)*10;
		...
		speed=1;
	     ...
	     {
	     if(but==239)BAT_C_POINT_1++;
	     else if(but==111)BAT_C_POINT_1=(BAT_C_POINT_1/10+1)*10;
	     else if(but==247)BAT_C_POINT_1--;
	     else if(but==119)BAT_C_POINT_1=(BAT_C_POINT_1/10-1)*10;
	     ...
	     {
	     if(but==239)BAT_U_END_1++;
	     else if(but==111)BAT_U_END_1=(BAT_U_END_1/10+1)*10;
	     else if(but==247)BAT_U_END_1--;
	     else if(but==119)BAT_U_END_1=(BAT_U_END_1/10-1)*10;
	     ...
	     {
	     if(but==239)BAT_C_POINT_1_2++;
	     else if(but==111)BAT_C_POINT_1_2=(BAT_C_POINT_1_2/10+1)*10;
	     else if(but==247)BAT_C_POINT_1_2--;
	     else if(but==119)BAT_C_POINT_1_2=(BAT_C_POINT_1_2/10-1)*10;
	     ...
	     {
	     if(but==239)BAT_U_END_1_2++;
	     else if(but==111)BAT_U_END_1_2=(BAT_U_END_1_2/10+1)*10;
	     else if(but==247)BAT_U_END_1_2--;
	     else if(but==119)BAT_U_END_1_2=(BAT_U_END_1_2/10-1)*10;
	     ...
	     {
	     if(but==239)BAT_C_POINT_1_6++;
	     else if(but==111)BAT_C_POINT_1_6=(BAT_C_POINT_1_6/10+1)*10;
	     else if(but==247)BAT_C_POINT_1_6--;
	     else if(but==119)BAT_C_POINT_1_6=(BAT_C_POINT_1_6/10-1)*10;
	     ...
	     {
	     if(but==239)BAT_U_END_1_6++;
	     else if(but==111)BAT_U_END_1_6=(BAT_U_END_1_6/10+1)*10;
	     else if(but==247)BAT_U_END_1_6--;
	     else if(but==119)BAT_U_END_1_6=(BAT_U_END_1_6/10-1)*10;
		...
		gran(&IZMAX,10,4000);
	     ...
	     {
	     if(but==239)UVENTOFF++;
	     else if(but==111)UVENTOFF+=2;
	     else if(but==247)UVENTOFF--;
	     else if(but==119)UVENTOFF-=2;
	     gran(&UVENTOFF,15,600);
	     lc640_write_int(0x10+100+240,UVENTOFF);
	     speed=1;
	     }
    else if(a_ind . s_i==26)
		{
		...
		if(but==254)
	     ...
	     {
	     if(but==239)BAT_K_OLD++;
	     else if(but==111)BAT_K_OLD+=10;
	     else if(but==247)BAT_K_OLD--;
	     else if(but==119)BAT_K_OLD-=10;
	     gran(&BAT_K_OLD,10,100);
	     lc640_write_int(0x10+350+94,BAT_K_OLD);
	     speed=1;
	     }						 		 			 		
 	}
...
void enmv_trap (void){
	  if(enmv_puts_en==1) { 
		if( ((enmv_data_pred[count_reg_enmv][cnt_enmv_modbus_adress]>>count_bit_enmv)&0x01) != ((enmv_data[count_reg_enmv][cnt_enmv_modbus_adress]>>count_bit_enmv)&0x01 )){  					   		
			char strt[40];	 
			if((enmv_data[count_reg_enmv][cnt_enmv_modbus_adress]>>count_bit_enmv)&0x01 ) {
				sprintf(strt,"BDI #%u, DI #%u Alarm",cnt_enmv_modbus_adress+1,count_reg_enmv*8+count_bit_enmv+1 );	
				snmp_trap_send(strt,21,count_reg_enmv*8+count_bit_enmv+1,1);				
			}	
			else {
				sprintf(strt,"BDI #%u, DI #%u Alarm is off",cnt_enmv_modbus_adress+1,count_reg_enmv*8+count_bit_enmv+1 ); 
				snmp_trap_send(strt,21,count_reg_enmv*8+count_bit_enmv+1,0);
			}						
		}

		if(count_bit_enmv<7) ++count_bit_enmv; 
		else {
			enmv_data_pred[count_reg_enmv][cnt_enmv_modbus_adress]=enmv_data[count_reg_enmv][cnt_enmv_modbus_adress];
			count_bit_enmv=0;
			if(count_reg_enmv<7) ++count_reg_enmv;
			else {count_reg_enmv=0; enmv_puts_en=0;}
...
if(((LPC_SC_TypeDef *) ((0x40080000UL) + 0x7C000) )->RSID&0x00000001L)bRESET_INT_WDT=1;

((LPC_SC_TypeDef *) ((0x40080000UL) + 0x7C000) )->RSID=((LPC_SC_TypeDef *) ((0x40080000UL) + 0x7C000) )->RSID;
...
if(__ee_spc_stat==spcVZ)
	...
	if(__ee_vz_cnt)
		...
		vz_cnt_h_=(lc640_read_int(0x10+500+150+14)-__ee_vz_cnt)/60;
		if(vz_cnt_h_<0)vz_cnt_h_=0;
		vz_cnt_s_=(short)(((lc640_read_int(0x10+500+150+14)-__ee_vz_cnt)*60)%3600UL);
...
socket_udp = udp_get_socket (0, 0x01 | 0x02, udp_callback);
if (socket_udp != 0) 
	{
...
if(lc640_read_int(0x10+450+30)!=bisOFF)lc640_write(0x10+450+30,bisOFF);
...
lc640_write_long(0x1800+4,199999999L);	
...
#line 44262 "main.c"
	...
	if(b1Hz)
	    ...
	    if(NUMENMV>0 && NUMENMV<9 && enmv_puts_en==0)  { 
			if(delay_enmv_puts>10) modbus_zapros_ENMV();
			else ++delay_enmv_puts;	
		}
