...
unsigned int sk1_24;
unsigned short Iddt_porog_pred, Iddt_porog_error; //rki_01 
unsigned char n_error_ddt_uku, u_rki;  //rki_01
unsigned short Rddt[8][4]; //rki_01
...
unsigned char ddt_error; // нет связи с дат тока   //rki_01
unsigned short status_izm_r;	// аварии измерения изоляции
unsigned int sk_alarm, status_di1, status_di2; // авария СК 
...
else if( num_rki && cnt_net_drv==-2) // запрос для РКИ	//rki_01_s
	{
     if(!bCAN_OFF)
		{		
...
/*
	...
	ptrs[6]=						" Umin шины      zB  ";
	if(type_rki==1){
		ptrs[7]=						" I ДДТ пред     qмА ";	
		ptrs[8]=						" I ДДТ авар     wмА ";	
		ptrs[9]=						" Выход              ";
	}
	else if(type_rki==0){
		ptrs[7]=						" Выход              ";
	...
	if(sk1_24&0x800000UL)ptrs[37]=	" СК24 ВКЛЮЧЕН       ";
 	...
 	ptrs[38]=						" Rпредв ДДТ     qкОм";
	ptrs[39]=						" Rавар ДДТ      wкОм";
	ptrs[40]=						" Авар. фидеров      ";
...
else if(ind==iK_RKI) // меню калибровки РКИ	 //rki_01_s
	{
	ret(1000);
	if(but==butD)
		{
...
else if(ind==iRKI) // индикация данных с РКИ	//rki_01_s
	{
	ret(1000);
	if(but==butD)
		{
...
/*	else if(but==butE)
		...
		if(but==butR)
			{
			speedChrgCurr++;
			gran(&speedChrgCurr,0,10000); //rki_01
			lc640_write_int(EE_SPEED_CHRG_CURR,speedChrgCurr);
			}
		else if(but==butR_)
			{
			speedChrgCurr+=10;	 //rki_01
			gran(&speedChrgCurr,0,10000);  //rki_01
			lc640_write_int(EE_SPEED_CHRG_CURR,speedChrgCurr);
			}
		else if(but==butL)
...
/*	
 ...
 if(sub_ind==0)
	    ...
	    else if(but==butR_)IMAX_VZ+=10;//((IMAX_VZ/10)+2)*10;	//rki_01
	    else if(but==butL)IMAX_VZ--;//((IMAX_VZ/10)-1)*10;
	    else if(but==butL_)IMAX_VZ-=10;//((IMAX_VZ/10)-2)*10;  //rki_01
		gran(&IMAX_VZ,10,10000); //rki_01	          
