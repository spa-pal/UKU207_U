...
//oleg_start*
else if( num_net_in && cnt_net_drv==18) // запрос для сетевого ввода
	{
     if(!bCAN_OFF)
		{
...
else if( num_rki && cnt_net_drv==19) // запрос для РКИ
	{
     if(!bCAN_OFF)
		{
...
//		{
    ...
    ptrs[3]=" 0%:0^:0& 0z/w  /0{ ";	  //oleg_start* заменил < на z, > на w.
	ptrs[4]="     Tб    ?°C      ";
/*     
 	ptrs[4]=										" Батарея N1         ";
    ptrs[5]=										" Батарея N2         ";
...
//    ptrs[5+NUMIST/*+NUMBAT+NUMINV*/]= 				" Нагрузка           "; 
	...
	//oleg_end далее во всех ptrs добавил +(num_rki!=0)+(num_net_in!=0)

    ptrs[6+NUMIST/*+NUMBAT+NUMINV*/+(num_rki!=0)+(num_net_in!=0)]=	" Внешние датчики    ";
	ptrs[6+NUMIST/*+NUMBAT+NUMINV*/+(NUMEXT!=0)+(num_rki!=0)+(num_net_in!=0)]= 	" Ускоренный заряд   ";
	if((speedChIsOn)&&(bFL))ptrs[6+NUMIST/*+NUMBAT+NUMINV*/+(NUMEXT!=0)+(num_rki!=0)+(num_net_in!=0)]= 	"                     "; 
 	ptrs[7+NUMIST/*+NUMBAT+NUMINV*/+(NUMEXT!=0)+(num_rki!=0)+(num_net_in!=0)]= 	" Спецфункции    	 ";
    ptrs[8+NUMIST/*+NUMBAT+NUMINV*/+(NUMEXT!=0)+(num_rki!=0)+(num_net_in!=0)]= 	" Установки          "; 
    ptrs[9+NUMIST/*+NUMBAT+NUMINV*/+(NUMEXT!=0)+(num_rki!=0)+(num_net_in!=0)]= 	" Журнал событий     "; 
    ptrs[10+NUMIST/*+NUMBAT+NUMINV*/+(NUMEXT!=0)+(num_rki!=0)+(num_net_in!=0)]= " Выход              "; 
    //ptrs[11+NUMIST/*+NUMBAT+NUMINV*/+(NUMEXT!=0)]= 	" Журнал батареи     "; 
    //ptrs[12+NUMIST+NUMBAT+/*NUMINV+*/(NUMEXT!=0)]= 	" Журнал батареи N2  "; 
	ptrs[11+NUMIST/*+NUMBAT+NUMINV*/+(NUMEXT!=0)+(num_rki!=0)+(num_net_in!=0)]=	" Тест               ";
	ptrs[12+NUMIST/*+NUMBAT+NUMINV*/+(NUMEXT!=0)+(num_rki!=0)+(num_net_in!=0)]=	" Таблица источников ";
...
//	if((AUSW_MAIN==22063)||(AUSW_MAIN==22023)||(AUSW_MAIN==22043))
	...
	//int2lcdyx(rki_avar1_ind_stat,0,19,0);
...
/*	int2lcdyx(bat_drv_rx_cnt,0,3,0);
	int2lcdyx(zTTButteryCnter,0,6,0);
...
//	int2lcd(TBATSIGN,'X',0);
 ...
 /*    ptrs[32]=      " Контроль ср.точки  ";
	...
	ptrs[50]=		"    &               ";
    ptrs[51]=		" РКИ                "; // oleg_start* вставлена строка
	ptrs[51+(num_rki!=0)]=						" Сетевые вводы      "; // oleg_start* вставлена строка
	ptrs[51+(num_rki!=0)+(num_net_in!=0)]=		" Выход              ";	// oleg_start* добавил +(num_rki!=0)
    ptrs[52+(num_rki!=0)+(num_net_in!=0)]=		" Калибровки         "; // oleg_start* добавил +(num_rki!=0)
    ptrs[53+(num_rki!=0)+(num_net_in!=0)]=		"                    "; // oleg_start* добавил +(num_rki!=0)       
...
// oleg_start*
	...
	if(ddt_error&0x000002){
		...
		ptrs[42]=						" Iддт2=      rмА    ";
		if(Rddt[0]==1001) 	ptrs[43]=	" Rиз2=     > 1МОм   ";
	...
	if(ddt_error&0x000004){
		...
		ptrs[44]=						" Iддт3=      tмА    ";
		if(Rddt[0]==1001) 	ptrs[45]=	" Rиз3=     > 1МОм   ";
	...
	if(ddt_error&0x000008){
		...
		ptrs[46]=						" Iддт4=      yмА    ";
		if(Rddt[0]==1001) 	ptrs[47]=	" Rиз4=     > 1МОм   ";
	...
	if(ddt_error&0x000010){
		...
		ptrs[48]=						" Iддт5=      uмА    ";
		if(Rddt[0]==1001) 	ptrs[49]=	" Rиз5=     > 1МОм   ";
	...
	if(ddt_error&0x000020){
		...
		ptrs[50]=						" Iддт6=      iмА    ";
		if(Rddt[0]==1001) 	ptrs[51]=	" Rиз6=     > 1МОм   ";
	...
	if(ddt_error&0x000040){
		...
		ptrs[52]=						" Iддт7=      oмА    ";
	   	if(Rddt[0]==1001) 	ptrs[53]=	" Rиз7=     > 1МОм   ";
	...
	if(ddt_error&0x000080){
		...
		ptrs[54]=						" Iддт8=      pмА    ";
		if(Rddt[0]==1001) 	ptrs[55]=	" Rиз8=     > 1МОм   ";
	...
	if(ddt_error&0x000100){
		...
		ptrs[56]=						" Iддт9=      aмА    ";
		if(Rddt[0]==1001) 	ptrs[57]=	" Rиз9=     > 1МОм   ";
	...
	if(ddt_error&0x000200){
		...
		ptrs[58]=						" Iддт10=     sмА    ";
		if(Rddt[0]==1001) 	ptrs[59]=	" Rиз10=    > 1МОм   ";
	...
	if(ddt_error&0x000400){
		...
		ptrs[60]=						" Iддт11=     dмА    ";
		if(Rddt[0]==1001) 	ptrs[61]=	" Rиз11=    > 1МОм   ";
	...
	if(ddt_error&0x000800){
		...
		ptrs[62]=						" Iддт12=     fмА    ";
		if(Rddt[0]==1001) 	ptrs[63]=	" Rиз12=    > 1МОм   ";
	...
	if(ddt_error&0x001000){
		...
		ptrs[64]=						" Iддт13=     gмА    ";
		if(Rddt[0]==1001) 	ptrs[65]=	" Rиз13=    > 1МОм   ";
	...
	if(ddt_error&0x002000){
		...
		ptrs[66]=						" Iддт14=     hмА    ";
		if(Rddt[0]==1001) 	ptrs[67]=	" Rиз14=    > 1МОм   ";
	...
	if(ddt_error&0x004000){
		...
		ptrs[68]=						" Iддт15=     jмА    ";
		if(Rddt[0]==1001) 	ptrs[69]=	" Rиз15=    > 1МОм   ";
	...
	if(ddt_error&0x008000){
		...
		ptrs[70]=						" Iддт16=     kмА    ";
		if(Rddt[0]==1001) 	ptrs[71]=	" Rиз16=    > 1МОм   ";
	...
	if(ddt_error&0x010000){
		...
		ptrs[72]=						" Iддт17=     lмА    ";
		if(Rddt[0]==1001) 	ptrs[73]=	" Rиз17=    > 1МОм   ";
	...
	if(ddt_error&0x020000){
		...
		ptrs[74]=						" Iддт18=     zмА    ";
		if(Rddt[0]==1001) 	ptrs[75]=	" Rиз18=    > 1МОм   ";
	...
	if(ddt_error&0x040000){
		...
		ptrs[76]=						" Iддт19=     xмА    ";
		if(Rddt[0]==1001) 	ptrs[77]=	" Rиз19=    > 1МОм   ";
	...
	if(ddt_error&0x080000){
		...
		ptrs[78]=						" Iддт20=     cмА    ";
		if(Rddt[0]==1001) 	ptrs[79]=	" Rиз20=    > 1МОм   ";
	...
	if(ddt_error&0x100000){
		...
		ptrs[80]=						" Iддт21=     vмА    ";
		if(Rddt[0]==1001) 	ptrs[81]=	" Rиз21=    > 1МОм   ";
	...
	if(ddt_error&0x200000){
		...
		ptrs[82]=						" Iддт22=     bмА    ";
		if(Rddt[0]==1001) 	ptrs[83]=	" Rиз22=    > 1МОм   ";
	...
	if(ddt_error&0x400000){
		...
		ptrs[84]=						" Iддт23=     nмА    ";
		if(Rddt[0]==1001) 	ptrs[85]=	" Rиз23=    > 1МОм   ";
	...
	if(ddt_error&0x800000){
		...
		ptrs[86]=						" Iддт24=     mмА    ";
		if(Rddt[0]==1001) 	ptrs[87]=	" Rиз24=    > 1МОм   ";
...
/*		else if(sub_ind==(2+NUMBAT+NUMIST))
		...
		else if((sub_ind==(3/*+NUMBAT*/+NUMIST/*+NUMINV*/)+(num_rki!=0)+(num_net_in!=0))&&(NUMEXT))	//oleg_start* добавил +(num_rki!=0)
			{
			tree_up(iExtern_220,0,0,0);
		     ret(1000);
			}

		else if(sub_ind==(3+NUMBAT+NUMIST+/*NUMINV+*/(NUMEXT!=0))+(num_rki!=0)+(num_net_in!=0))	 	//oleg_start* добавил +(num_rki!=0)
			{
			speedChargeStartStop();
			}

		else if(sub_ind==(4+NUMBAT+NUMIST+/*NUMINV+*/(NUMEXT!=0))+(num_rki!=0)+(num_net_in!=0))	 	//oleg_start* добавил +(num_rki!=0)
			{
			tree_up(iSpc_termocompensat,0,0,0);
		     ret(1000);
			}

		else if(sub_ind==(5+NUMBAT+NUMIST+/*NUMINV+*/(NUMEXT!=0))+(num_rki!=0)+(num_net_in!=0))	 	//oleg_start* добавил +(num_rki!=0)
			{
			tree_up(iSet_prl,0,0,0);
		     ret(50);
		     parol_init();
			}
		else if(sub_ind==(6+NUMBAT+NUMIST+/*NUMINV+*/(NUMEXT!=0))+(num_rki!=0)+(num_net_in!=0))		//oleg_start* добавил +(num_rki!=0)
			{
			if(but==butE_)avar_uout_hndl(1);
			tree_up(iLog,0,0,0);
		     ret(1000);
			}
		else if(sub_ind==(7+NUMBAT+NUMIST+/*NUMINV+*/(NUMEXT!=0))+(num_rki!=0)+(num_net_in!=0))		//oleg_start* добавил +(num_rki!=0)
			{
			sub_ind=0;
			}
	/*	else if(sub_ind==(8+NUMBAT+NUMIST+NUMINV+(NUMEXT!=0)))
...
/*		else if(sub_ind==(9+NUMBAT+NUMIST+NUMINV+(NUMEXT!=0)))
		...
		else if(sub_ind==(8+NUMBAT+NUMIST+/*NUMINV+*/(NUMEXT!=0))+(num_rki!=0)+(num_net_in!=0))		//oleg_start* добавил +(num_rki!=0)
			{
			if(but==butE)
		     	{
		     	tree_up(iPrltst,0,0,0);
		...
		else if(sub_ind==(9+NUMBAT+NUMIST+/*NUMINV+*/(NUMEXT!=0))+(num_rki!=0)+(num_net_in!=0))	 	//oleg_start* добавил +(num_rki!=0)
			{
			if(but==butE)
		     	{
		     	tree_up(iBps_list,0,0,0);
...
/*	else if(sub_ind==32)
	...
	if(but==butD)
        ...
        if(sub_ind==48)
        	{
            sub_ind=49;
			index_set=49; // oleg_start
		    }
        if(sub_ind==50)
        	{
            sub_ind=51;
		    }																	
		gran_char(&sub_ind,0,52+(num_rki!=0)+(num_net_in!=0)); //oleg_start* было	 gran_char(&sub_ind,0,52);
       ...
       /* if(sub_ind==39)
		...
		{
		sub_ind=51+(num_rki!=0)+(num_net_in!=0); // oleg_start* было 51
	 ...
	 else if(sub_ind==3)  // oleg_start*
		{
		if(but==butE) tree_down(0,0); ret(0);			 
		}				  // oleg_end*
	else if(sub_ind==4)
	     {
		if(ZV_ON)ZV_ON=0;
	...
	// oleg_end	
	else if(sub_ind==52)// oleg_start* было 51
		{
		if(but==butE)
		     {
...
/**/    else if(sub_ind==5)	  /**/
	...
	if(but==butD)
		{
		sub_ind++;
		gran_char(&sub_ind,0,6); // oleg_start* было 4
		}
	else if(but==butU)
		{
		sub_ind--;
 ...
 // oleg_end		  			  			                 
    else if(sub_ind==6)	 // oleg_start* было 4
	     {
	     if(but==butE)
	          {
			tree_down(0,0);
...
/**/	   	else if(sub_ind==(2+(NUMBAT!=0)+(NUMIST!=0)+(NUMINV!=0)+2))	   /**/
	...
	if(but==butD)
		{
		sub_ind++;
		gran_char(&sub_ind,0,3+(NUMIST!=0)+(NUMDT!=0)+(num_rki!=0)+(num_net_in!=0)); //oleg_start*
		}
	else if(but==butU)
		{
		sub_ind--;
